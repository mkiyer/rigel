#!/usr/bin/env python
"""What is the true per-transcript fragment count, split by splicedness? One pass over an oracle BAM.

A per-transcript prior claims to know, before the EM runs, how many fragments transcript `t` emitted
and how many of them were spliced; this script derives that truth from the simulator's read names alone.
The simulator writes ``{t_id}:{start}-{end}:{strand}:{index}`` with the interval in spliced-transcript
coordinates, so a fragment is spliced exactly when its interval crosses one of its transcript's interior
cumulative-exon boundaries. Splicedness is never read from the CIGAR: a junction that falls in the
unsequenced inner gap between the mates leaves no ``N`` in either mate, so a CIGAR-based count is biased
in one direction only. The count emitted is what the simulator produced, not what the resolver
concluded — the EM's own "not unspliced" bit is wider, since a clean-CIGAR fragment whose mate gap
admits more than one hypothesis is promoted to implicit-spliced, and the two must not be scored against
each other without saying so (`TRAPS: score-the-consumers-own-count`). Exact-duplicate transcripts,
which the index collapses onto one representative, are folded onto the retained twin before counting.
Seven named gates check the result against the condition's truth files and the index; no solver runs.

Usage::

    python scripts/design/transcript_truth.py --condition gdna_g00_ss_0.99_nrna_mid_capture_off   # count and gate one condition
    python scripts/design/transcript_truth.py --condition C --out truth_by_transcript.tsv          # also write the per-transcript table
    python scripts/design/transcript_truth.py --condition C --limit 200000                         # a smoke run; the gates are skipped
    python scripts/design/transcript_truth.py --suite SUITE --index IDX --gtf GENES.gtf --condition C
"""
...
def duplicate_map(gtf: Path) -> dict[str, str]:
    ...
    The index collapses transcripts sharing a reference, a strand and an identical sorted exon tuple
    onto the lexicographically smallest id, because they are unidentifiable from any read data. The
    simulator samples from the un-collapsed annotation, so its read names can reference ids the index
    does not contain; folding those onto the retained twin is the only count consistent with what the
    index decided. Rebuilt from the GTF because the index does not record the map.
    """
    import collections
    import re
...
def interior_boundaries(index: TranscriptIndex) -> dict[str, np.ndarray]:
    ...
    one of the gates below rather than an assumption.

    The exons are reversed for a minus-strand transcript: transcript coordinates run 5'→3' while the
    exons are stored genomically ascending. ``transcripts.feather`` encodes strand as ``{1: POS, 2: NEG}``
    and not as ``±1``; reading it as a sign silently disables this reversal.
    """
...
def count_condition(bam: str, bounds: dict[str, np.ndarray], limit: int | None = None,
    ...
    counting records would double every fragment and counting unique qnames would hold every string.
    """
    import pysam

    per_t: dict[str, list[int]] = {}
    # A transcript the read names reference but the index does not contain. Counting such a fragment
    # as "unspliced" would be a silent default on a substrate inconsistency, so it is reported apart.
...
def gates(res: dict, suite: Path, condition: str, index: TranscriptIndex,
    ...
    if col is None:
        ...
        # undefined — the two transcripts are the same molecule and only the group total is a fact.
        ta = ta.copy()
