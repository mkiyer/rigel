"""Should a fragment's 1/L be PARTITIONED across the edges it crosses?

Three candidate rules, each evaluated at every edge against the KNOWN true density rho:
...
def run(spacing):
    ...
    for _ in range(N_REP):
        ...
        # edges strictly inside the fragment: cuts[j] with start < cuts[j] < end
        ...
        for s, e, L, a, b, k in zip(starts, ends, lens, lo, hi, K):
            if k == 0:
                continue
            idx = np.arange(a, b) - 1  # edge index (cut j -> edge j-1)
            ok = (idx >= 0) & (idx < n_edge)
            idx = idx[ok]
            if idx.size == 0:
                continue
