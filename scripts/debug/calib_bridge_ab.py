...
def calib_obs(pl, fl):
    return np.asarray(calibrate(pl, ra, sm, fl.gdna_pmf, fl.rna_pmf, cfg).mass_gdna_contained, float)
...
for cond in conds:
    ...
    os.environ["RIGEL_MIX_BRIDGE"] = "0"
    off = calib_obs(pl, fl)
    os.environ["RIGEL_MIX_BRIDGE"] = str(eps)
    on = calib_obs(pl, fl)
    os.environ["RIGEL_MIX_BRIDGE"] = "0"
