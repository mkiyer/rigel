...
AMBIG (both-strand) regions a purely-local solver cannot resolve, which the production simplex sweep
resolves by propagating per-strand RNA:gDNA odds in from strand-informative neighbours. Each locus is
defined in LOCAL coordinates and placed in its own genome window; the harness runs the production
calibrator and scores per-locus mass-weighted AMBIG-node error vs the oracle. Extend by appending to
LOCI; re-run to track the solver improving.
...
def main():
    ...
    with tempfile.TemporaryDirectory() as work:
        ...
        def est(Kg=None):
            # K is driven by CalibrationConfig.sweep_n_grid (the production simplex-sweep path).
            extra = {"sweep_n_grid": Kg} if Kg else {}
            cfg = dataclasses.replace(PipelineConfig().calibration, **extra)
        ...
        for name, _desc, _g in LOCI:
            ...
            for i in sel:
                tot = og[i] + on[i] + om[i]
                if tot < 10 or not np.isfinite(e[i]):
                    continue
                orc = og[i] / tot
                le += abs(e[i] - orc) * tot; w += tot
            if w == 0:
                print(f"  {name:>12} {len(sel):>4}   (no AMBIG mass)"); continue
            T += le
            print(f"  {name:>12} {len(sel):>4} {le:>9,.0f}")
        print(f"  {'TOTAL':>12}      {T:>9,.0f}")
        sc.cleanup()
