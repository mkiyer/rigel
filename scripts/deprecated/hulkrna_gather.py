...
SAMTOOLS_RMDUP_STATS_FILE = 'samtools_rmdup_stats.json'
...
def parse_samtools_rmdup_stats(lib_dir):
    filename = lib_dir / SAMTOOLS_RMDUP_STATS_FILE
    d = {}
    with open(filename, 'r') as f:
        data = json.load(f) 
        d['rmdup_total_reads'] = int(data['READ'])
        d['rmdup_excluded'] = int(data['EXCLUDED'])
        d['rmdup_duplicates'] = int(data['DUPLICATE TOTAL'])
        d['rmdup_duplicate_pct'] = round(100.0 * float(d['rmdup_duplicates']) / d['rmdup_total_reads'], 2)
        d['rmdup_est_lib_size'] = int(data['ESTIMATED_LIBRARY_SIZE'])
    return d
...
def parse_metadata(lib_dir):
    ...
    d.update(parse_samtools_rmdup_stats(lib_dir))
