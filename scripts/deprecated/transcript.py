...
class Strand(IntEnum):
    ...
    def from_str(cls, s):
        try:
            return cls(cls.__STR_TO_ENUM[s])
        except KeyError:
            raise ValueError(f"Invalid strand string: {s!r}")

    def to_str(self):
        return self.__ENUM_TO_STR[self]
    
    @classmethod
    def from_is_reverse(cls, is_reverse):
        '''Convert is_reverse flag to Strand enum'''
        if is_reverse:
            return cls.NEG
        else:
            return cls.POS
    ...
    def opp(self):
        if self == Strand.POS:
            return Strand.NEG
        elif self == Strand.NEG:
            return Strand.POS
        elif self == Strand.AMBIGUOUS:
