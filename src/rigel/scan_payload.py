...
* ``boundary_flux`` :: uint32[B_obj_total, 4]
    ...
    def from_scan_result(cls, scan_result: dict[str, Any]) -> "AccumulatorPayload":
        ...
        if cal is None:
            raise ValueError(
                ...
                "BamScanner.set_regions was not called"
            )
        ...
        if n_ch != N_CHANNELS:
            raise ValueError(
                f"calibration payload has n_channels={n_ch}, expected {N_CHANNELS}"
            )
        ...
        ref_region_offsets = np.ascontiguousarray(
            cal["ref_region_offsets"], dtype=np.int64
        )
        ref_boundary_offsets = np.ascontiguousarray(
            cal["ref_boundary_offsets"], dtype=np.int64
        )

        if ref_pos_offsets.shape != (n_refs + 1,):
            raise ValueError(
                f"ref_pos_offsets shape {ref_pos_offsets.shape} != ({n_refs + 1},)"
            )
        if ref_region_offsets.shape != (n_refs + 1,):
            raise ValueError(
                f"ref_region_offsets shape {ref_region_offsets.shape} != "
                f"({n_refs + 1},)"
            )
        if ref_boundary_offsets.shape != (n_refs + 1,):
            raise ValueError(
                f"ref_boundary_offsets shape {ref_boundary_offsets.shape} != "
                f"({n_refs + 1},)"
            )
        if int(ref_pos_offsets[-1]) != boundaries.shape[0]:
            raise ValueError(
                "ref_pos_offsets[-1] does not match boundaries length"
            )

        r_total = int(ref_region_offsets[-1])
        b_total = int(ref_boundary_offsets[-1])

        region_contained = np.ascontiguousarray(
            cal["region_contained"], dtype=np.uint32
        ).reshape(r_total, N_CHANNELS)
        ...
        boundary_flux = np.ascontiguousarray(
            cal["boundary_flux"], dtype=np.uint32
        ).reshape(b_total, N_CHANNELS)

        # Invariant: each ref with k regions contributes (k, k+1) or (0, 0).
        expected_b_total = r_total + int(
            np.sum(np.diff(ref_region_offsets) > 0)
        )
        if expected_b_total != b_total:
            raise ValueError(
                f"R_total + nonempty refs ({expected_b_total}) != "
                f"B_obj_total ({b_total})"
            )
