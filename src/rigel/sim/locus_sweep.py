...
def parse_gtf(gtf_path):
    ...
    with open(gtf_path) as fh:
        for line in fh:
            ...
            transcripts[t_id]["exons"].append([int(fields[3]) - 1, int(fields[4])])
            if gene_id:
    ...
    for strand in sorted(by_strand):
        ...
        for start, end, t_id in intervals:
            if merged and start <= merged[-1][1]:
                ...
                merged[-1] = (prev_start, max(end, prev_end), prev_ids + [t_id])
            else:
...
def compute_nrna_groups(transcripts_config, user_groups=None):
    ...
    dict[str, NrnaGroup]
    """
    t_spans = {t_id: _transcript_span(t_def) for t_id, t_def in transcripts_config.items()}
    ...
    if user_groups:
        for label, t_ids in user_groups.items():
            ...
            groups[label] = NrnaGroup(label, list(t_ids), strand, start, end, carrier)
    ...
    if remaining:
        for grp_ids, strand, start, end in _group_overlapping(remaining, t_spans):
            if len(grp_ids) == 1:
            ...
            groups[label] = NrnaGroup(label, grp_ids, strand, start, end, carrier)
...
def parse_nrna_coords(nrnas_config, transcripts_config):
    ...
    for label, coords in sorted_nrnas:
        if not isinstance(coords, list) or len(coords) != 3:
            raise ValueError(f"nrna '{label}': expected [strand, start, end], got {coords!r}")
        strand = str(coords[0])
        ...
        groups[label] = NrnaGroup(
            label=label, t_ids=matching, strand=strand, start=start, end=end, carrier=carrier
        )
    ...
    if remaining:
        for grp_ids, strand, start, end in _group_overlapping(remaining, t_spans):
            ...
            groups[lbl] = NrnaGroup(
                label=lbl, t_ids=grp_ids, strand=strand, start=start, end=end, carrier=carrier
            )
...
def _parse_params_section(params_config):
    ...
    for section_key, field_set in _PARAM_SECTIONS.items():
        ...
        for k, v in section.items():
            if k not in field_set:
                logger.warning("Unknown param '%s' in params.%s — ignoring", k, section_key)
                continue
    ...
    for k, v in params_config.items():
        ...
        if k in _ALL_PARAM_FIELDS:
            ...
            logger.warning("Unknown param '%s' in params section — ignoring", k)

    return param_dims


def build_sweep_grid(sweep_config, patterns_config, all_t_ids, nrna_labels, param_dims=None):
    ...
    if patterns_config:
        ...
        if conflict:
            raise ValueError(
                f"Entities cannot appear in both 'sweep' and 'patterns': {sorted(conflict)}"
            )
...
def run_sweep(config, output_dir, *, gtf_path=None, genome_length_override=None, dry_run=False):
    ...
    if gtf_path:
        ...
        raise SystemExit(
            "No transcript definitions: provide 'transcripts' in YAML or --gtf on the command line"
        )
    ...
    if nrnas_config:
        ...
        if user_groups is None and gtf_gene_groups:
            user_groups = {f"nrna_{gid}": t_ids for gid, t_ids in gtf_gene_groups.items()}
    ...
    dims, linked_names = build_sweep_grid(
        sweep_config, patterns_config, all_t_ids, nrna_labels, param_dims=param_dims
    )
    ...
    result_cols = (
        ...
        + ["cal_density_est"]
        + ["cal_gdna_fl_true_mean", "cal_gdna_fl_est_mean", "cal_gdna_fl_mean_err"]
    ...
    with open(tsv_path, "w", newline="") as tsvfile:
        ...
        for run_idx, combo in enumerate(combinations):
            ...
            logger.info("=" * 60)
            logger.info("Run %d/%d: %s", run_idx + 1, len(combinations), run_label)

            n_frags = int(params.get("n_fragments", 2000))
            n_rna_frags = int(params["n_rna_fragments"]) if "n_rna_fragments" in params else None
            gdna_frac = float(params["gdna_fraction"]) if "gdna_fraction" in params else None
            ...
            # meaningless.
            all_rna_zero = all(float(params.get(t, 0)) == 0 for t in all_t_ids) and all(
            ...
            with tempfile.TemporaryDirectory(prefix="rigel_sweep_") as tmpdir:
                ...
                em_kwargs = {k: params[k] for k in _EM_FIELDS if k in params and k != "seed"}
                scan_kwargs: dict = {"sj_strand_tag": "auto"}
                scan_kwargs.update({k: params[k] for k in _SCAN_FIELDS if k in params})
                scoring_kwargs = {k: params[k] for k in _SCORING_FIELDS if k in params}
                ...
                bench = run_benchmark(
                    result,
                    pr,
                    scenario_name=f"sweep_{run_idx}",
                )
                ...
                for t_id in all_t_ids:
                    ...
                    if ta:
                        ...
                        for suf in ("_expected", "_observed", "_abs_diff", "_rel_err"):
                            row[f"{t_id}{suf}"] = 0
                ...
                nrna_rel = (
                    bench.nrna_abs_diff / bench.n_nrna_expected
                    if bench.n_nrna_expected > 0
                    else 0.0
                )
                ...
                mrna_rel = (
                    abs(bench.total_observed - bench.total_expected) / bench.total_expected
                    if bench.total_expected > 0
                    else 0.0
                )
                ...
                rna_rel = abs(trna_obs - trna_exp) / trna_exp if trna_exp > 0 else 0.0
                ...
                if rna_m.n_observations > 0:
                    ...
                    row["rna_fl_mean_err"] = round(rna_m.mean - rna_true_mean, 2)
                else:
                    row["rna_fl_est_mean"] = ""
                    row["rna_fl_est_std"] = ""
                    row["rna_fl_mean_err"] = ""
                ...
                if gdna_m.n_observations > 0:
                    ...
                    row["gdna_fl_mean_err"] = round(gdna_m.mean - gdna_true_mean, 2)
                else:
                ...
                cal_kappa_true = float(gdna_sk) if gdna_sk is not None else ""
...
""",
    ...
    parser.add_argument("-c", "--config", required=True, help="YAML sweep configuration file")
    parser.add_argument(
        "-g",
        "--gtf",
        default=None,
        help="GTF file for transcript structures (overrides 'transcripts' in YAML)",
    )
    parser.add_argument(
        "-L", "--genome-length", type=int, default=None, help="Genome length in bp (overrides YAML)"
    )
    parser.add_argument(
        "-o", "--outdir", default="sweep_results", help="Output directory (default: sweep_results)"
    )
    parser.add_argument("--dry-run", action="store_true", help="Show sweep grid without running")
