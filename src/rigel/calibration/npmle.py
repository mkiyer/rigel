...
1. **Enrichment NPMLE → σ²_transfer** (:meth:`~DensityNPMLE.project`). Fit on every node's **TOTAL** unspliced
   ...
   (`docs/calibration/archive/transfer_variance_formal_derivation.md`). This is the only NPMLE running in the INITIAL solve; it never
...
Design (docs/calibration/archive/npmle_struggles.md §8-9, docs/calibration/archive/npmle_roadmap.md; docs/calibration/archive/CALIBRATION_MASTER.md §5 for the two roles):
...
def _kde_density(log_rho, a_cells, w_cells, h, *, a_floor=-np.inf, w_floor=0.0, h_floor=None):
    ...
    ``docs/calibration/archive/gdna_kde_restore_plan.md``): ``Σ_c w_c·N(grid; a_c, h) + w_floor·N(grid; a_floor, h_floor)`` → a
...
class DensityNPMLE:
    ...
    def fit(
        ...
        *,
        ...
        ``additive=True`` builds the **Role-B representation** (``docs/calibration/archive/gdna_kde_restore_plan.md``) instead of the
        ...
        # ΣE. This is the hybrid the design calls for (docs/calibration/archive/CALIBRATION_MASTER.md §5B; gdna_background_floor_derivation
        ...
        if additive:
            # ── THE ROLE-B ADDITIVE KDE (docs/calibration/archive/gdna_kde_restore_plan.md) — occupancy kernels + a weak floor, NO EM. ──
    ...
    def logprior(self, fg_grid, mass, eff):
        ...
        This is the ``ψ_λ = strand + logP_g + logP_r`` result (`docs/calibration/archive/prior_ramp_and_bp_roadmap.md` §2). The
        ...
        **The background enters SMOOTHLY, never as a clamp** (`docs/calibration/archive/CALIBRATION_MASTER.md` §5B). When this prior was
    ...
    def project(self, mass, eff, *, chunk: int = 8192):
        ...
        `docs/calibration/archive/npmle_projection_variance_design.md` §1 role 2). The observed density is soft-assigned to the mixture
