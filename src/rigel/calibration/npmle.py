...
1. **Enrichment NPMLE → σ²_transfer** (:meth:`~DensityNPMLE.project`). Fit on every node's **TOTAL** unspliced
   ...
   (`transfer_variance_formal_derivation.md`). This is the only NPMLE running in the INITIAL solve; it never
...
Design (docs/calibration/npmle_struggles.md §8-9, npmle_roadmap.md; CALIBRATION_MASTER.md §5 for the two roles):
...
def _kde_density(log_rho, a_cells, w_cells, h, *, a_floor=-np.inf, w_floor=0.0, h_floor=None):
    ...
    ``gdna_kde_restore_plan.md``): ``Σ_c w_c·N(grid; a_c, h) + w_floor·N(grid; a_floor, h_floor)`` → a
...
class DensityNPMLE:
    ...
    def fit(
        ...
        *,
        ...
        ``additive=True`` builds the **Role-B representation** (``gdna_kde_restore_plan.md``) instead of the
        ...
        # ΣE. This is the hybrid the design calls for (CALIBRATION_MASTER.md §5B; gdna_background_floor_derivation
        ...
        if additive:
            # ── THE ROLE-B ADDITIVE KDE (gdna_kde_restore_plan.md) — occupancy kernels + a weak floor, NO EM. ──
    ...
    def logprior(self, fg_grid, mass, eff):
        ...
        **The background enters SMOOTHLY, never as a clamp** (`CALIBRATION_MASTER.md` §5B). When this prior was
    ...
    def project(self, mass, eff, *, chunk: int = 8192):
        ...
        `npmle_projection_variance_design.md` §1 role 2). The observed density is soft-assigned to the mixture
