...
* **boundary** is observable ⇔ **no exon bit is shared** across its two sides → no single
  ...
  transcript spanning the boundary would put unspliced mature RNA there). Its crossing
...
* **run interiors** (consecutive non-observable regions, no observable side): the nearest anchored
  neighbour, carried inward from the run's observable boundaries (forward + reverse, averaged).
...
class RegionDensity:
    ...
    boundary_count_observable: np.ndarray  # bool[E] — count-observable contiguous boundary
...
def region_gdna_density(chain, geometry, region_arrays) -> RegionDensity:
    ...
    ``eff_gdna`` — the contained placements at a REGION slot, the crossing placements at an BOUNDARY slot.
    One divisor rule, one source, so a caller can no longer hand this a length model that disagrees
    with the one the solver uses. The flanking boundaries are found through the chain's own adjacency, so
    ...
    def flank(neighbour: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        """The gDNA density at a region's flanking BOUNDARY, and whether that boundary can anchor it.

        ⭐ **The boundary is reached through the chain's own adjacency**, and its observability is read on
        the BOUNDARY axis at that slot's ``obj_idx`` — so "which boundary is to my left" is answered by the
        ...
        edge_idx = np.clip(obj[safe], 0, max(boundary_count_observable.shape[0] - 1, 0))
        observable = (
            usable & boundary_count_observable[edge_idx]
            if boundary_count_observable.size
    ...
    # count-observable and are imputed from the boundaries below; the strand for an exon enters the
    ...
    # Everything else: anchor from the available observable boundaries — the AVERAGE of whichever sides are
