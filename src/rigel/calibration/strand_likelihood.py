...
Both components carry a fitted overdispersion with the same default prior, so they collapse to the same
distribution under sparse data and an unstranded region (κ = ½, equal overdispersions) is
uninformative. A gDNA-only Beta-Binomial breaks that symmetry: its ``−½·log var`` term prefers the
lower-variance component and pulls balanced regions toward RNA.

Given the observed (sense, antisense) split, this returns the strand log-likelihood over a grid of
candidate gDNA fractions ``gdna_frac``, which the per-region strand module multiplies into the count
prior. Only POS/NEG regions carry a valid sense split; AMBIG (both strands) and intergenic (no
transcript) regions are strand-uninformative and left to the count clue.
"""
...
def strand_loglik(
    ...
    *,
    ...
    """Two-component gDNA/RNA strand log-likelihood of one region over a ``gdna_frac`` grid.
