...
class BackgroundReference:
    ...
    n_regions: int = (
        0  #: number of pooled REGIONS — the aggregate cell's EM weight (the population it stands for)
    )
...
def measure_background(
    ...
    *,
    ...
    ``region_eff_len`` is ``effective_length.contained_eff_length`` on the gDNA pmf — the count of start
    positions that place a whole molecule inside the region, which is the support the counts are a rate over.
