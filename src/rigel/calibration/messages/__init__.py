...
class StepContext:
    ...
    right: np.ndarray
    is_edge: np.ndarray  # ~is_node; the chain strictly alternates N E N E … N
    is_exon_node: np.ndarray  # a NODE whose region signature is EXON — the graft's destination
    ...
    geometry: object  # NodeGeometry, for the frame pair (a policy-owned derivation)
    ...
    own: object  # NodeInit: the message-free self-solve — rho_*, prec_*, tau_lam, struct_lock, f_*
    belief_fg: (
        np.ndarray
