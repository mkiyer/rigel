...
class NodeGeometry:
    ...
    # (docs/CARRY_FORWARD.md §4.1); inert until that lands.
...
def _boundary_strand_stats(boundary_substrate, region_arrays):
    ...
    The allow mask is the **transcript-structure CONTINUITY gate** (`docs/CARRY_FORWARD.md`):
    ...
    (contiguous exon on both flanks) that selects the per-node prior (`docs/CARRY_FORWARD.md` §3).
    """
...
class NodeStatics:
    ...
    exon) that select the per-node solver prior (`docs/CARRY_FORWARD.md` §3). All ``float64`` except the four
