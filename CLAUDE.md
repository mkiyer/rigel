...
| `transfer` | ⭐⭐ **RUNGS 1+2+3 of the ground-up rebuild** (`messages/transfer.py`, owner rulings 2026-09-01/02, LADDER-CONFIRMED) — COMPOSITION TRANSFER, one hop, nothing relayed, silence-not-zeros. Rung 1: the intron's factory row VERBATIM at intron\|exon boundaries. Rung 2: the same row transported into EXONS through the monotone FACE MAP (certified flux CAPS the claimable gDNA share) and widened by the face's own counting variance. Rung 3: the intergenic\|exon EDGE's sign-certified LOWER BOUND (the profile likelihood over enrichment ≥ 1; a zero edge is VACUOUS by law — ⛔ the enrichment-ceiling upper side is owner-REFUSED as over-engineering and the zero-gDNA edge residual is an ACCEPTED error). ⭐ Rung 1 COMPLETED 2026-09-02 (item 1): the EXON → intron\|exon boundary message — the exon's own strand row (deadband-gated) read at the GEOMETRIC splice-in map (`f_b = f_E (U+S)/U`, the enrichment ratio cancels; capture-blind opportunities for BOTH components), marginalised over the face's measured ratio; ladder: unstranded byte-identical, stranded ON 0.987–0.995×. No constants anywhere; face-local ratios only, so no level crosses a capture cliff. ≤ silent on 7/8 stranded ladder rows, −68…−76 % on the blind rows, best-or-near-best on BOTH adversarial probe panels where the relay's anchor is catastrophic. ⭐ Rung 2 COMPLETED 2026-09-02 (item 2): the intron\|exon BOUNDARY → INTRON message — the boundary's OWN strand row VERBATIM (one shared population, no splice-out, no premise; the s = 0 map is the identity), deadband-gated at the boundary; stage 0 certified the shared composition on all four substrates; ladder unstranded byte-identical, stranded ≤ silent 7/8, −26…−38 % at the receiving introns under capture; ⛔ judge a message at its DESTINATIONS — the whole-library number carries the refit prior's response (`DESIGN.md` §6b.5). ⭐ Rung 4 item 5 SHIPPED 2026-09-02: the exon\|exon TERMINUS boundary and its OUTSIDE exon (the flag alone orients; ⚠ the licence counts the boundary's SPLICED crossing — `f_b = f_O (U_b+S_b)/U_b`, item 1's map — the verbatim form was REFUTED on certified truth); three messages, the composed transport among them, no-echo structural; ladder unstranded 0.985× at the g00 zero control, stranded ≤ shipped 7/8, below silence 8/8 (`DESIGN.md` §6b.6). ⛔ The census verdict: exon\|exon boundaries and walled exons hold most of the remaining error but sit at the resolution of their own evidence plus the prior — the lever is the landscape prior, not another message. ⭐ Rung 4 item 6 SHIPPED 2026-09-02 — THE OWNER'S ABUNDANCE-DISCREPANCY RULE (`DESIGN.md` §6b.7): into the INSIDE flank of a terminus the boundary's own row travels the map `f_X = f_c·s/r`, the step `s` between enrichment (composition transfers) and new RNA (the gDNA abundance transfers), never above `f_c`, its spread FITTED from the served pairs' two witnesses — the precision set by the discrepancy in total abundance, first do no harm; ladder stranded ≤ shipped 5/6, below silence 16/16, unstranded byte-identical. ⭐ Items 2, 5, 6 and 7 landed 2026-09-02 (`DESIGN.md` §6b.5–§6b.8): TEN messages ship, the last at the ALTERNATIVE SPLICE SITE, where every message carries THE HOP PREMISE — a step fitted per hop kind from the sighted pairs with its error, and the owner's discrepancy rule per pair (`transfer_rows.hop_step_fit`; the scan's per-hop dampening template). Standing shadows: deferred `g05 ss.50 ON`, the prior-mediated `g05 ss.99 ON` 1.005× and the other recorded owner decisions in the handoff, and `ISSUES: gdna-landscape-trains-on-false-positives` |
...
⭐ **THE STANDING BASELINE: 0 failed / 3,780 passed / 0 skipped / 8 xfail** (re-derived
2026-09-02 after item 7 of the message rungs landed). Account it from **3,776** — the count after
items 2, 5 and 6 — by **+4**: the four item-7 gates (the flank predicate, the hop premise's fit and
shift, the landing, the deadband), all added to the EXISTING `tests/calibration/test_transfer_policy.py`
(no file was added, so no parametrised gate moved; the `src/` edits touched existing modules and add no
case). ⛔ **RE-DERIVE, NEVER ADJUST** — the table below gives the per-file deltas, and a bracket-matched
`--collect-only` confirms the attribution (3,788 collected).
