...
⭐ **THE STANDING BASELINE IS GREEN: 0 failures, 3,554 passing, 0 SKIPPED, 9 xfail, 3,563 collected**
(re-derived 2026-08-19: `preflight.py`, the session's first command). ⚠ The `+4` over 3,559 is the
measured `+4` rule for ONE new `scripts/design/` file, re-derived with
`pytest --collect-only -q | grep preflight`. ⚠ Before it, `benchmark_report.py` was the same `+4`
over 3,555.
...
| `design/build_scan_cache.py` | scan once, calibrate many times. ⛔⛔ **THE SCHEMA DIGEST DOES NOT SEE A DEPOSIT-RULE CHANGE.** A cache is keyed by the index, graph hash, reach digest, payload SCHEMA digest and scan CONFIG — **none of which describes what the accumulator DEPOSITS** — so a content-changing scanner fix leaves every key identical and the rebuild is a SILENT NO-OP that reports success (measured 2026-08-19: `cached / skip` on all 32 after the chimera repair). Use `--force` (wired through `panel.py` 2026-08-19) or delete the caches by hand. ⛔ `rescan_panels.py` is NOT the tool for this class of change: it gates shared banks on BYTE-IDENTITY, which is right for a rename and stops on every condition when the counts legitimately move |
