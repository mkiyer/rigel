...
> **Calibration (acyclic, per-node gradient strand/count combine).** The calibration stage is an **acyclic single-pass** deconvolution that combines, per node, a **strand module** (Beta-Binomial posterior — the gDNA fraction from the strand *direction*, capture-invariant) and a **count module** (local gDNA-density imputation on **RAW contained + STRAND-CLEANED boundaries** — count *magnitude*) by `g = w·g_strand + (1−w)·g_count` with a **per-node strand-trust gradient** `w = I/(I+I₀)`, `I = N·(2κ−1)²` (per-fragment discriminability × the node's unspliced count `N`), `I₀ = 10` (`config.gdna_strand_info_scale`). `w→1` where the strand is informative (high κ **and** depth — its own deconvolution governs); `w→0` at κ≈½ / thin / AMBIG (the count imputation governs) — a gradient, not a gate. Cleaning only the **boundaries** (the contained stays raw — the strand enters once, via `g_strand`) keeps `g_strand` and `g_count` independent → no double-count. Not a joint product (the joint count×strand approach is archived in `docs/calibration/archive/joint_deconvolution.md`). No iterative EM/M-step. (Redesign history — the combine was briefly retired then restored as this gradient after a forensic flagship investigation — is in `docs/calibration/redesign_phase3_plan.md`.) Theory: `docs/calibration/calibration_theory.md` (authoritative); fractional-accumulator spec in `docs/accumulator/00_design.md`; open work in `docs/calibration/CALIBRATION_TODO.md`; docs index in `docs/README.md`. The full pipeline (scan → calibrate → **quant**) runs end-to-end: `quant_from_buffer` + `calibration.priors.assemble_priors` are wired in `run_pipeline`.
...
- `calibrate.py` — the **acyclic single-pass** calibrator orchestrator (RNA strand balance κ → gDNA/RNA strand overdispersion → `strand_deconvolve` cleans the boundary crossings → count module `g_count` = imputation on raw contained + cleaned boundaries + 3-term splice → `deconv_regions`/`deconv_sides` per-node gradient combine `g=w·g_strand+(1−w)·g_count`, `w=I/(I+I₀)`, `I=N·(2κ−1)²`, `I₀=10` → derive)
