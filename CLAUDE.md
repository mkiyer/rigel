...
| **Stage B — calibration** | |
| `sim/configs/gdna_ladder.yaml` | ⭐ **the Stage-B panel, and the ONLY panel on disk** — **16 conditions** (`g00/g05/g50/g98` × ss `0.50/0.99` × capture off/on), EQUAL fragment lengths, gDNA 0 → 98 % at a fixed 10 M total. ⛔ **REBUILT FROM SCRATCH 2026-08-13**, when `pilot`, `flgap_short` and `flgap_long` were deleted and the ladder went 36 → 16; `g05` survives because `suite_resolves.py`'s requirement (f) tests the RATE and demands one rung in `0 < rate ≤ 0.10`. ⚠ The ORACLE CACHE beside it at `ladder/oracle_cache/` holds **12 of 16** BY DESIGN — `pass0_vs_oracle.py` holds out every zero-gDNA condition, so `panel.py status` prints ✘ on a complete panel. `docs/TESTING.md` §0 |
