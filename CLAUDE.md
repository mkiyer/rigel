...
**The standing baseline: 0 failed / 3,338 passed / 0 skipped / 2 xfail, 3,340 collected** — re-derived 2026-09-24 on branch
`calibrated-likelihood` with the pseudocount's count form and the gDNA-candidate rule (3,307 − 5 retired prior tests
+ 2 + 18 for `tests/test_em_pseudocounts.py` + 18 in `tests/test_pipeline_routing.py`), measured with two `docs/dev/`
...
| `design/prior_vs_oracle.py` | ⭐⭐⭐ **IS `LocusPriors` — what the EM's prior reads from calibration — RIGHT?** Calibration's per-locus gDNA count and gDNA length; five arms separate calibration's own error from the assembler's, per stratum, in the drained frame. The EM reads the count against its own fragment count (`pipeline.em_pseudocounts`), so the count's error is the whole of the prior's |
