...
| `design/length_channel_census.py` | ⭐⭐⭐ **DOES THE FRAGMENT-LENGTH CHANNEL KNOW ANYTHING TRUE? — no solver runs.** `length_likelihood` has never been switched on, so a panel A/B would price the channel, the relay, the prior and the solver at once. This isolates it: the SHIPPED moments on the SHIPPED geometry, evaluated on the real per-slot `(count, inv_length_sum, length_sum)`, argmax scored against the origin-split oracle's TRUE `f_g` at the same slot, stratified by the count `N`. ⭐ Three questions in order — does it FIRE, is the covariance CONDITIONED (`u=1/w` and `w` are the same draw, so `rho→−1` by construction), is the claim TRUE and the precision EARNED. ⛔ Carries its own null: ONE pmf on both arms must be EXACTLY inert. ⛔ **Its verdict is that the channel is faithful but NOT READY** — see `docs/dev/length_channel.md` |
| `design/short_exon_fl_probe.py` | ⭐⭐ **CAN A SHORT EXON GIVE AN UNBIASED RNA LENGTH SAMPLE? — offline, no accumulator.** A fragment touching an INTERNAL exon shorter than itself cannot be contained, so it is spliced with probability exactly 1 — length bias removed by geometry rather than corrected. Built entirely from the simulator's read names (`{t_id}:{frag_start}-{frag_end}` in TRANSCRIPT coordinates) and the index's exon structure, so a schema change can be justified by measurement before it is spent. ⛔ **Its verdict on the generalised form (`e < w` per fragment) is NEGATIVE: mean 239.44 against a true 212.20**, because the number of qualifying exons itself grows with `w`. `--max-exon` runs the narrower fixed-set form, which is untested. ⭐⭐ Its PREMISE GATE is the reason to keep it: read-name spans vs the condition's own truth table — that gate is what exposed `truth_fragment_lengths.tsv`'s `kind` column as nested AGGREGATES (`rna` and `all` re-count `mrna`), which had corrupted a whole diagnosis |
| `design/length_likelihood_ab.py` | ⭐⭐⭐ **THE ON/OFF A/B FOR `length_likelihood`, AGAINST REAL TRUTH.** One thing varied (the config flag) on the same cached scan; scores the library `f_gdna` against each condition's own truth from the origin-split caches, and reports the mass with NO own composition evidence beside it. Reads a flat cache root or the oracle-cache layout (`<cond>/_main`). ⛔⛔ **ALWAYS RUN THE `g00` ARM**: on flgap (all `g50`) the channel looks like an 87 % win and at `g00` it reports 54–57 % gDNA in a library with none. ⛔⛔ **AND ITS `f_gdna` IS THE WRONG POPULATION ON THE WRONG ARM** (measured 2026-08-10): it sums every slot with no mask, so it is a count-weighted mean of per-slot `f_g` over the UNDETERMINED population too; it never pins `calib_refit_iters`, so the arm is the SHIPPED calibration and not pass-0; and it omits `mass_rna_junction`, which `pipeline.py` includes — 5,048,297 fragments at `g00 ss0.50 capture_on`, worth −0.192 of the headline alone. Score the channel on `solvability_audit.py` and `prior_vs_oracle.py` instead. ⚠ Its `~.52` truth placeholder was replaced with the measured value — a placeholder cannot be subtracted from |
| `design/local_mean_length.py` | ⭐⭐⭐ **IS THE MODEL-FREE LOCAL MEAN LENGTH TRUE, AND WHAT DOES IT BUY? — no solver runs.** At a contiguous EDGE the `w−1` opportunity and the `1/(w−1)` deposit cancel for ANY pmf, so `count/inv_length_sum + 1 = E[w]` with no length model. Scores that per component against the origin-split oracle's own realised mean, with the NODE axis as its falsification (the `(ell−w+1)₊` opportunity does NOT cancel, and it reads **+22.8 % / +32.1 %** there against **+0.66 %** at an EDGE, gDNA and RNA tracking to 2 % — which localises the node failure to the OPPORTUNITY, since gDNA cannot splice). ⛔ Its verdict on the INVERSION is NEGATIVE and the docstring says so first: the identity is exact and the inversion `phi = (E[w]−mu_r)/(mu_g−mu_r)` is ill-conditioned, so every ladder residual exceeds the unit interval `phi` must live in. ⚠ `--drain` is opt-IN and production DRAINS |
| `design/length_gap_sweep.py` | ⭐⭐⭐ **IS THE LENGTH CHANNEL'S ANSWER A FUNCTION OF THE PMF GAP AT ALL? — DRAINED by default, no solver runs.** Interpolates `gdna_pmf` from `rna_pmf` to its shipped value over TWELVE orders of magnitude and reports amplitude, participation, the answer and the declared precision separately, because the whole finding is that they do not fade together. ⛔⛔ **Its verdict: NO, and closing the gap makes it WORSE.** At a gap of ~1e-9 bp the channel reports **0.72 / 0.59 / 0.72** on libraries whose truths are **0.00 / 0.00 / 0.57**, and growing the gap to its real value improves all three. ⭐ Participation is a CLIFF — `alpha = 0` gives 0 live slots, `alpha = 1e-12` gives **100.00 %** of the mass — while amplitude fades linearly, so the exact-`!=` guard switches while the signal fades. ⛔ This is the instrument that exonerates the pmf and kills every "close the gap" repair. Gated on both endpoints (`alpha = 0` exactly inert, `alpha = 1` bit-identical to the shipped channel) with a swapped-endpoint perturbation that must break the second |
| `design/length_salvage_ab.py` | ⭐⭐⭐ **CAN THE LENGTH CHANNEL BE SALVAGED? — the P1 SILENCE and P2 SPEECH gates, IN THE SOLVER.** The offline instruments score the row's value; this scores its ADMISSIBILITY, which is what the feature turns on. Arms `off` / `as_is` / `marg` (the covariance marginalised over pmf uncertainty, `N·V + N²·Σ`, INSIDE `length_loglik` so the mode moves too) / `marg_frozen` (the same, held at `V(½)`, so the row is exactly quadratic and exactly constant at `Δ = 0`). ⭐ `Σ` needs no constant: sampling from `pool_total + prior_ess` by the delta method, and ⭐⭐ **placement from the FOUR gDNA POOLS' own disagreement** — four estimates of one distribution that must agree under uniform placement, so their spread measures `EQUATIONS.md` §4.4's residual with no oracle, on real data. ⛔ **Its verdict is that P1 FAILS on every arm (92–100 % reclassification) and the cause is located: admissibility is a BOOLEAN (`ptp > 0`) while information is a continuum.** ⭐ Gated: `Σ→0` must reproduce the shipped row bit-for-bit, and both salvaged arms must be exactly inert under one pmf |
| `design/length_row_shape.py` | ⭐⭐⭐ **WHAT SHAPE IS THE LENGTH ROW, AND WHICH REBUILD OF IT GOES INERT AS THE GAP GOES TO ZERO? — no solver runs.** Splits the shipped row into `−½ quad` and `−½ log det` (gated: the split must reproduce `length_loglik` to ~1e-9), reports the argmax distribution mass-weighted, and prices seven rebuilds against per-slot truth. ⛔ **Its verdict refutes the log-det hypothesis**: `quad` owns the argmax on **99.34 %** of NODE and **99.95 %** of EDGE mass, and at an EDGE the log-det term's `ptp` is a constant 5.4e-04 nats — the row is a saturating monotone ramp, not a flat row and not a middle mode. ⛔ And NOT ONE arm produces a positive slope against truth across g00/g50/g98 |
| **plumbing** | |
