...
from hulkrna.bam import parse_bam_file, parse_read
...
MULTIMAP_BAM_FILE = 'multimap.bam'
...
class Fragment(NamedTuple):
    ...
    refs: Set[str]
    strand: Strand
    exons: Set[Tuple[str, int, int, Strand]]
    sjs: Set[Tuple[str, int, int, Strand]]


def merge_fragment_reads(r1, r2):
    '''Merge alignment information from paired reads r1 and r2'''
    refs = set()
    exon_dict = collections.defaultdict(list)
    sjs = set()
    strand = Strand.NONE
    ...
    for read in (r1, r2):
        ...
        refs.add(rref)
        k = (rref, rstrand)
        # TODO: need to track read 1 and read 2
        # probably need to flip strand dfor one or the other
        exon_dict[k].extend(rexons)
        sjs.update((rref, start, end, sj_strand) for start, end, sj_strand in rsjs)
    ...
    for (eref, estrand), intervals in exon_dict.items():
        ...
        for start, end in intervals[1:]:
            if start <= cur_end:
                ...
                merged_exons.add((eref, cur_start, cur_end, estrand))
                cur_start, cur_end = start, end
        merged_exons.add((eref, cur_start, cur_end, estrand))
    return Fragment(refs, strand, merged_exons, sjs)
...
def hulkrna_count(input_bam_file, index_dir, output_dir,
    ...
    # save multimap reads to separate bam file
    multimap_bam_file = output_dir / MULTIMAP_BAM_FILE

    with (
        pysam.AlignmentFile(input_bam_file, "rb", threads=threads) as bamfh, 
        pysam.AlignmentFile(multimap_bam_file, "wb", template=bamfh) as mm_bamfh
    ):
        ...
        for r1, r2 in parse_bam_file(
            ...
            fragment = merge_fragment_reads(r1, r2)
    ...
    with pysam.AlignmentFile(input_bam_file, "rb", threads=threads) as bamfh:
        multimap_bam_file = output_dir / MULTIMAP_BAM_FILE
        with pysam.AlignmentFile(multimap_bam_file, "wb", template=bamfh) as multimap_bamfh:
            # parse bam file and return paired reads
            logging.info(f'[START] Parsing BAM file {input_bam_file}')
            i = 0
            for r1, r2 in parse_bam_file(
