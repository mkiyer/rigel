...
``boundary_unspliced_count`` is ``+1`` on every boundary a fragment crosses, so a fragment books
``max(K, 1)`` of them and a sum over objects is an object-INCIDENCE count rather than a fragment count.
The prior the EM reads is a fragment count, and manufacturing one from a density over-calls it under
capture; the mass bank carries the count directly, one fragment depositing exactly one. Conservation
alone is far too weak to pin the rule — a ``1/K`` share conserves exactly too — so this module makes
four claims that are not the same claim:

===  ==========================================================================================
 1   a fragment's deposits sum to ONE, to within the float64 representation
 2   the mass equals the PER-BASE attribution, which ``1/K`` cannot reproduce because a base does
     not know how many boundaries its fragment crossed. This is the claim that pins the rule
 3   a spliced fragment's mass reaches the SPLICED bank and nothing else
 4   every crossed object SHARES the one fragment, sj boundaries included
===  ==========================================================================================

Claim 2 states the rule a second time, deliberately and in one place only: the specification reaches
the answer through slices and two ``searchsorted`` calls, while the per-base walk here asks of every
base "which region holds you, and which of that region's bounding positions lie strictly inside this
fragment?". Agreement between two spellings of one rule is not automatic (TRAPS: a-test-that-redefines).
...
#: The fixture, and every part of it is load-bearing. Boundaries 1 and 5 have BOTH flanks wider than
#: every fragment below — the control, where a crossing fragment can only cross that one boundary.
#: Boundaries 2, 3 and 4 bound a cluster of 50 bp regions, so one 300 bp fragment crosses three at once.
#: Under capture that cluster is where most of the gDNA crossings are, so it is the geometry that
#: matters rather than a corner case.
...
def _mass_by_base(region_bounds, start: int, end: int) -> dict[int, Fraction]:
    ...
    Every base is asked which region holds it; the region's two bounding positions are boundaries iff
    they lie strictly inside the fragment; the base is split equally between the boundaries that do
    bound it. Divided by the fragment's length, that is the mass — reached without ever forming a
    slice.
...
def test_the_RULE_conserves_exactly_before_any_quantisation():
    ...
    Every unspliced path is ONE segment. If it stays inside a region it is ``contained``. Otherwise it
    has ``n >= 2`` slices: the first crosses right only, the last crosses left only, every interior one
    crosses both. EVERY SLICE HAS AT LEAST ONE BOUNDING BOUNDARY, so ``Σ slice_len / L = 1`` exactly
    and nothing is dropped by the "no boundary here" guard.

    Stated on the RULE, through the per-base attribution, and NOT on the bank, which cannot represent a
    third exactly. Those are two different claims and the next test makes the other one. Conflating
    them is how a representation residue gets mistaken for a modelling error, or worse, how a real
    error gets absorbed into a tolerance that was sized for rounding.
    """
    for length in LENGTHS:
        for start in range(REGION_BOUNDS[0], REGION_BOUNDS[-1] - length + 1):
...
def test_the_BANK_conserves_to_within_the_REPRESENTATION_and_no_more():
    ...
    The worst observed drift is asserted as a RANGE of its own, because a budget that is never
    approached says nothing about whether it is the right budget: zero would mean the representation
    changed, and more than the budget would mean the deposits are not round-to-nearest.
    """
    ...
    # A few ulp at worst, asserted as a RANGE for the reason the docstring gives.
...
def test_a_CONTAINED_fragment_deposits_NO_mass_at_all():
    """Its whole fragment is already ``region_contained_count``; mass at a boundary as well would be
    it counted twice. This is the branch the law's ``contained`` term exists for."""
    partition = _partition()
...
def test_the_two_populations_together_are_EXHAUSTIVE_for_an_unspliced_fragment():
    """A contiguous interval either stays inside one region or crosses at least one boundary — so over
    the enumeration, ``Σ contained + Σ mass`` is the fragment COUNT, with no third case to account for.

    At the bank's precision: the budget is the total number of crossings, one rounding each, by the
...
# ---------------------------------------------------------------------------
# claim 2 — the VALUE, which is what separates this rule from 1/K
# ---------------------------------------------------------------------------
...
def test_the_mass_is_the_PER_BASE_attribution():
    ...
    splits each base between the boundaries bounding its own region, and no per-base rule can produce
    ``1/K`` because a base carries no knowledge of how many boundaries the whole fragment crossed.

    PERTURBATION: injecting an exact ``1/K`` deposit into the specification leaves every other gate in
    this module green — both conservation gates, the exhaustiveness sum, and even the wide-flank closed
    form, where ``K == 1`` makes the two rules identical — and fails only this one.

    The tolerance is the representation and nothing else: each of a boundary's ``<= 2 * count``
    deposits is rounded to nearest, so the bank can differ by at most that many half-ulp. Not a fitted
    tolerance (TRAPS: no-magic-numbers).
    """
    partition = _partition()
...
def test_the_mass_EQUALS_the_count_where_both_flanks_exceed_every_fragment():
    """The closed form, and the reason a per-object sum reads as a fragment count off capture and not
    under it.
...
def test_a_SPLICED_fragment_s_mass_reaches_the_SPLICED_bank_ALONE():
    """A spliced fragment is certified RNA at that boundary — gDNA cannot splice — so its mass must
    never enter the unspliced competition the prior arbitrates.

    The geometry is the staggered case the second bank exists for: the fragment splices over one exon
    boundary and then runs CONTIGUOUSLY across another isoform's boundary inside its second block.
    """
    ...
    # A PARTIAL, and the exact one the rule specifies: the boundary takes only the bases adjacent to it,
    ...
    #
    # Asserting 200/300 for the boundary would encode the rule in which a boundary-crossing block gives
    # its bases ENTIRELY to boundaries, leaving the sj that bounds it with nothing.
    assert spliced < 1
    ...
    # And together they are the whole fragment.
...
def test_the_spliced_mass_is_a_PARTIAL_and_never_a_conservation_ledger():
    ...
    reading a consumer can make, so it is pinned rather than left to a docstring."""
    region_bounds = [0, 1000, 2000, 9000, 9050, 10000]
    partition = Partition.from_region_bounds(
...
# ---------------------------------------------------------------------------
# claim 4 — the law over ALL THREE axes, which is what makes a LIBRARY count computable
# ---------------------------------------------------------------------------

#: The staggered geometry claim 3 uses: one annotated sj, and a short region (9000–9050) whose far
...
def test_a_spliced_fragment_crossing_NO_BOUNDARY_still_deposits_a_WHOLE_FRAGMENT_of_mass():
    """The population a boundary-only conserved count cannot see: both blocks lie inside one region
    each.
    ...
    On a contaminated library it is a quarter of the RNA side and none of the gDNA side, because gDNA
    cannot splice — which is why the gDNA side's conserved count is complete without this bank and the
    RNA side's is not.
    """
...
#: TWO sj, and the middle exon is a WHOLE region. A one-sj fixture cannot exercise the equal-share
#: rule at all — every block it produces is bounded on one side only, so ``len(bounds)`` is always 1
#: and a rule that forgot to share passes every gate (TRAPS: could-the-arm-have-fired).
...
def test_a_block_bounded_by_TWO_sj_SPLITS_its_mass_between_them():
    """The claim that pins the sj rule, the way claim 2 pins the boundary rule.
    ...
    # SUMMED over the strand columns. `sj_mass` is per strand for artifact detection, and CONSERVATION
    # is a strand-agnostic property — a fragment's 1.0 is shared across the objects it touched whatever
    # strand it aligned to — so every gate in this file reads the total.
...
def test_a_fragment_crossing_BOTH_sj_AND_boundaries_gives_EVERY_crossed_object_a_share():
    """A spliced fragment may cross several sj AND several boundaries, and all of them share the one
    fragment.

    Conservation alone does not imply sharing. A rule that gives a boundary-crossing block's bases
    entirely to boundaries leaves a sj flanked by two boundary-crossing blocks with exactly zero while
    ``sj_count`` credits it — and the fragment still sums to 1.0, so every conservation gate passes.
    On real geometry that empties a small but non-zero set of crossed sj. This gate asserts the
    property conservation cannot see.
...
def test_a_sj_claims_at_BOTH_its_positions_and_never_ALSO_as_a_contiguous_crossing():
    """The case that discriminates where conservation cannot.
    ...
    A rule that gave a boundary-crossing block's bases entirely to boundaries scores 200/200 here,
    because the sj never collects on its ACCEPTOR side. Both rules sum to 1.0, so no conservation gate
    can tell them apart; this one can, which is why the case is pinned.

    And a sj must not ALSO register as a contiguous crossing — that would be the same traversal
    counted twice. It cannot, structurally: a boundary is crossed iff it lies STRICTLY inside a block,
    and a sj's donor and acceptor are block endpoints. Asserted rather than assumed.
    """
...
def test_EVERY_deposited_fragment_places_exactly_ONE_fragment_of_mass_spliced_or_not():
    ...
    Claim 1 states it for unspliced fragments only, and that scoping is correct rather than an
    oversight — the boundary rule alone genuinely does not hold across a sj. This is the same law once
    the sj axis carries its share, and it is the identity a library fragment count rests on.

    The enumeration sweeps the second block's end across the short region's far boundary, so it
    contains spliced fragments that cross a boundary and spliced fragments that cross none, plus the
    unspliced population for the control.

    The budget is the representation and nothing else, by claim 1's derivation: every deposit is
    rounded to nearest, so ``n`` fragments making ``d`` deposits drift by at most ``d/2`` ulp. Not a
