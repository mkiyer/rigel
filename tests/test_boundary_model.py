...
def _strand_channels(boundary_right_mean: list[float]) -> RegionGdnaChannelEstimate:
    ...
    return RegionGdnaChannelEstimate(
        ...
        contained_reliability=np.ones(3, dtype=np.float32),
        contained_log_bayes_factor=zeros.copy(),
        boundary_left_reliability=np.ones(3, dtype=np.float32),
        boundary_left_log_bayes_factor=zeros.copy(),
        boundary_right_reliability=np.ones(3, dtype=np.float32),
