...
def _write_minimal_index(tmp_path: Path, t_df: pd.DataFrame) -> Path:
    ...
    # The splice graph: one INTERGENIC region tiling chr1, zero edges. Required at load, so the
    ...
    _, regions, edges = build_index_artifacts([], {"chr1": 1000})
    ...
    t_df = pd.DataFrame(
        {
            ...
            "t_index": [0, 2],
            "g_index": [0, 0],
            "g_name": ["TRAPS: no-magic-numbers", "TRAPS: no-magic-numbers"],
            "g_type": ["pc", "pc"],
            "is_basic": [True, True],
            "is_mane": [False, False],
            "is_ccds": [False, False],
