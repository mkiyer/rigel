"""Shared fixtures and assertion helpers for the oracle-mode scenario tests.

Every scenario here is built with ``Scenario.build_oracle()``, whose alignments are perfect by
construction, so a failure is the pipeline's and never an aligner's and the suite needs no external
tools. The assertion helpers below are shared because each one encodes a tolerance that has to mean
the same thing in every scenario that uses it.
"""
...
def assert_negative_control(bench, ctrl_id="t_ctrl", *, gdna_abundance=0, strand_specificity=1.0):
    """A negative-control transcript must carry near-zero mRNA counts.

    The tolerance scales with gDNA contamination and with the strand-specificity gap, and it is
    referenced to PERFECT strand specificity: any value below 1.0 carries real orientation ambiguity,
    so the silent-control limit has to grow with that gap as well as with gDNA. At imperfect strand
    specificity and moderate contamination an honest gDNA-to-RNA residual of a few percent survives
    calibration, and this bound admits it rather than hiding it behind an over-called gDNA fraction.
    """
