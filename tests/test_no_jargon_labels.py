"""A rule is cited by its name, never by a number, and this is what makes that true.

`TRAPS.md`'s rules were once headed `A16`, `D4j`, `C0b`; they are named now, so a citation says what it
means without a lookup and stays one greppable string with one home. The numbers were not merely opaque,
they were ambiguous — one token meant a rule in `TRAPS.md` and also a success criterion, a moment
variable in `effective_length.py`, or a structurally pure-gDNA object — and a name cannot collide that
way. A convention would let them back one docstring at a time, which is exactly how they spread, so this
scans every `.py`, `.md`, `.h` and `.cpp` file in the tree and every real collision is exempted by exact
file rather than in bulk.
"""
