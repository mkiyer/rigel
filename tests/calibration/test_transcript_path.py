...
def test_a_MULTI_EXON_transcript_takes_its_exons_and_a_junction_between_them(
...
def test_a_transcript_with_no_exons_contributes_an_EMPTY_path(tmp_path, _patched_junctions):
...
def test_a_MINUS_strand_path_runs_in_TRANSCRIPTION_order(tmp_path, _patched_junctions):
    ...
    # ⭐ and the two sj ids DIFFER, because strand is part of the junction key — two transcripts on
    ...
    assert [o for k, o in fwd if k == STEP_SPLICE_JUNCTION] != [
        o for k, o in rev if k == STEP_SPLICE_JUNCTION
    ]
...
def test_two_transcripts_sharing_one_intron_resolve_to_the_SAME_sj_id(
    ...
    """⭐ A junction is a property of the genome, not of a transcript: two isoforms splicing the same
    ...
    path = build_transcript_path(idx, _Regions(bounds))
    sj0 = [o for k, o in _kinds(path, 0) if k == STEP_SPLICE_JUNCTION]
    sj1 = [o for k, o in _kinds(path, 1) if k == STEP_SPLICE_JUNCTION]
    assert sj0 == sj1 == [0]
...
def test_an_UNRESOLVABLE_intron_RAISES_rather_than_dropping_a_step(tmp_path, _patched_junctions):
    ...
    bounds = [0, 1_000, 2_000, 9_000, 10_000, 11_000]
    idx = _Index(tmp_path, bounds, {0: [(1_000, 2_000), (9_000, 10_000)]}, strands=[Strand.POS])
    idx.edges_df = idx.edges_df.iloc[:0]  # the junction axis no longer carries this intron
...
def test_the_CSR_is_well_formed(tmp_path, _patched_junctions):
    bounds = [0, 1_000, 2_000, 9_000, 10_000, 11_000]
    idx = _Index(
