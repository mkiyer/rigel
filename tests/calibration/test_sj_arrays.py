...
intron an annotated sj, and if so which boundary?* Getting it wrong is not a rounding error — an
intron that fails to match deposits nothing at all (an unannotated intron credits no sj boundary), so
