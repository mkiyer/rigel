...
2026-07-29: the ``ambig_dense_10mb`` annotation has ZERO mergeable adjacencies, so its v8 node
...
#: the signature does not change there, which is precisely why the retired v7 merge deleted this cut.
GTF_ALT_TSS = """\
...
#: The SAME annotation with t1's alternative start removed (it now shares t0's) — one node fewer,
#: and the ONLY difference is the cut a signature merge cannot see. Measured under v7: the two
...
# ═══════════════════════════════════════════════════════════════════════════════════════════════
# The arm's own gate: the scanner sees the cut the merge deletes.
# ═══════════════════════════════════════════════════════════════════════════════════════════════
...
def test_the_cut_is_signature_invisible(alt_tss_index):
    ...
    assert nodes["signature"].iloc[i] == nodes["signature"].iloc[i - 1] == BIT_EXON_POS
    assert int(nodes["end"].iloc[i - 1]) == ALT_TSS_POS
...
def test_scanner_partition_carries_the_alternative_tss(alt_tss_index):
    ...
    positions, _offsets, _types = build_node_partition_arrays(alt_tss_index)
    assert ALT_TSS_POS in set(positions.tolist())


def test_scanner_partition_is_exactly_the_node_cut_array(alt_tss_index):
    """The emitted triple IS the v8 graph, per reference, in ``ref_names`` order — not a
    re-derivation of it that could drift."""
    positions, offsets, types = build_node_partition_arrays(alt_tss_index)
    nodes = alt_tss_index.nodes_df

    n_refs = len(alt_tss_index.ref_names)
    assert offsets.shape == (n_refs + 1,)
    assert int(offsets[-1]) == positions.shape[0]
    # k nodes per reference ⇒ k+1 cut positions; region_types is 1:1 with nodes.
    assert positions.shape[0] == len(nodes) + n_refs
    assert types.shape[0] == len(nodes)

    for i, ref in enumerate(alt_tss_index.ref_names):
        sub = nodes[nodes["ref_name"] == ref]
    ...
    np.testing.assert_array_equal(types, coarse_type_array(nodes["signature"].to_numpy()))


def test_region_geometry_follows_the_scanner_partition(alt_tss_index):
    """``RegionArrays.from_index`` and ``build_node_partition_arrays`` must read the SAME frame.
    ...
    nodes = alt_tss_index.nodes_df
    assert ra.n_regions == len(nodes)
    np.testing.assert_array_equal(ra.start, nodes["start"].to_numpy(np.int64))
    np.testing.assert_array_equal(ra.end, nodes["end"].to_numpy(np.int64))
    np.testing.assert_array_equal(ra.signature, nodes["signature"].to_numpy(np.uint8))

    positions, offsets, _types = build_node_partition_arrays(alt_tss_index)
    # k+1 cut positions per reference against k regions: the payload's own boundary/region relation.
    np.testing.assert_array_equal(
        np.diff(offsets.astype(np.int64)) - 1, np.diff(ra.ref_offsets.astype(np.int64))
    )


def test_adjacent_nodes_may_share_a_signature(alt_tss_index):
    ...
    signature. v8 drops exactly that — 59.5 % of human transcript termini fall at a cut where the
    signature does not change, so the merge was deleting the cut that made them visible.
    """
    ra = RegionArrays.from_index(alt_tss_index)
...
def test_partition_hash_distinguishes_a_signature_invisible_cut(alt_tss_index, no_alt_tss_index):
    """⭐ Two annotations differing by exactly one signature-invisible cut must not share a cache key.
    ...
    and it was live until the partition became the node cut array.
    """
    assert len(alt_tss_index.nodes_df) == len(no_alt_tss_index.nodes_df) + 1
