...
accumulator deposits into: the v8 region cut array. :meth:`RegionArrays.from_index` reads the same
...
def test_scanner_partition_is_exactly_the_region_cut_array(alt_tss_index):
    ...
    # k regions per reference ⇒ k+1 cut positions; region_types is 1:1 with regions.
...
def test_partition_hash_distinguishes_a_signature_invisible_cut(alt_tss_index, no_alt_tss_index):
    ...
    and it was live until the partition became the region cut array.
    """
