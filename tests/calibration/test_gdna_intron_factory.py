"""Unit tests for the gDNA intron factory primitive (docs/calibration/gdna_intron_factory_design.md).
...
from rigel.calibration.gdna_intron_factory import (
    IntronBackground,
    _log_negbinom,
    intron_lambda_factor,
)
...
def test_factor_peaks_at_rho_bg_over_rho_obs():
    ...
    fac = intron_lambda_factor(_bg(rho_bg), C, np.array([E_g]), _GRID)
...
def test_regime_dna_free_pins_low():
    ...
    fac = intron_lambda_factor(bg, C, np.array([1000.0]), _GRID)
...
def test_precision_monotone_in_count():
    ...
    def width(C):
        fac = intron_lambda_factor(_bg(rho_bg), np.array([C]), np.array([E_g]), _GRID)[0]
...
def test_overdispersion_widens_the_peel():
    ...
    def width(alpha):
        fac = intron_lambda_factor(_bg(rho_bg, alpha=alpha), C, np.array([E_g]), _GRID)[0]
        w = np.exp(fac - fac.max())
        w /= w.sum()
        m = (w * _GRID).sum()
...
def test_non_informative_is_flat():
    ...
    fac = intron_lambda_factor(bg, np.array([100.0, 5.0]), np.array([1000.0, 1000.0]), _GRID)
...
def test_factor_shape_and_finiteness():
    ...
    fac = intron_lambda_factor(bg, C, Eg, _GRID)
