...
⭐ **The structural fact.** An BOUNDARY is a single genomic position and it counts the fragments spanning it
CONTIGUOUSLY, so the transcripts it can see are exactly those continuous across it::

    T(BOUNDARY)  =  T(REGION_left)  ∩  T(REGION_right)

A transcript whose body begins at the BOUNDARY is in the right flank and not in the BOUNDARY; one that ends there
is in the left flank and not in the BOUNDARY. So a transcript TERMINUS at the BOUNDARY is precisely what makes one
flank's population strictly larger, and the licence is an **equality** of population per (BOUNDARY, side)
...
transcript's UP from its TES. So::
    ...
    the RIGHT flank gains RNA at an BOUNDARY  <=>  a transcript's genomic LOW  end is there
                                          <=>  FLAG_TSS_POS  or  FLAG_TES_NEG
    the LEFT  flank gains RNA at an BOUNDARY  <=>  a transcript's genomic HIGH end is there
...
must give the **same** flank answer, while the TSS bit alone points at the opposite BOUNDARIES. A predicate
...
treatments, and extending this licence to splice sites is a separate experiment (`ISSUES: the-cancelling-pair`).
...
#: the gene is 7 REGIONS and 6 BOUNDARIES with no intron anywhere and the RNA population is a strict staircase.
#: Every BOUNDARY therefore carries a terminus, and which FLANK it implicates is known by hand:
#:
...
def test_the_flank_that_gains_is_the_one_the_transcript_SPANS_say_it_is(
    ...
    structural BITS off the graph; this recomputes ``T(BOUNDARY) = T(left) ∩ T(right)`` from the transcript
    ...
    for e, p in enumerate(pos):
        ...
        assert bool(right_gains[e]) == (t_boundary != t_right), (
            f"@{p}: right_gains={bool(right_gains[e])} but T(BOUNDARY)={sorted(t_boundary)} vs "
            f"T(right)={sorted(t_right)}"
        )
        assert bool(left_gains[e]) == (t_boundary != t_left), (
            f"@{p}: left_gains={bool(left_gains[e])} but T(BOUNDARY)={sorted(t_boundary)} vs "
...
def test_the_flank_answer_is_the_hand_written_one(which, pos_index, neg_index):
    ...
    index = pos_index if which == "pos" else neg_index
    right_gains, left_gains = terminus_flank_gain(build_boundary_flags_array(index))
    pos = _boundary_positions(index)
...
def test_THE_STRAND_MIRROR_gives_the_SAME_flanks_while_the_TSS_BIT_POINTS_THE_OTHER_WAY(
    ...
    carries RNA the BOUNDARY cannot see is the SAME one in both: it is a fact about which coordinate direction
    ...
    the **opposite** BOUNDARIES between the two indexes. So a predicate written in TSS/TES terms would be
    exactly inverted on one of them — silently, since both are valid annotations."""
    p_flags = build_boundary_flags_array(pos_index)
    n_flags = build_boundary_flags_array(neg_index)
    p_pos, n_pos = _boundary_positions(pos_index), _boundary_positions(neg_index)
    ...
    assert np.array_equal(p_right, n_right), (
        f"the RIGHT-gaining BOUNDARIES differ between the mirrors: {p_pos[p_right]} vs {n_pos[n_right]}"
    )
    assert np.array_equal(p_left, n_left), (
        f"the LEFT-gaining BOUNDARIES differ between the mirrors: {p_pos[p_left]} vs {n_pos[n_left]}"
    )
...
def test_a_flagless_boundary_breaks_no_population():
    """⭐ The empty case, stated so a future all-zeros plumbing bug cannot read as "everything matches".
    ``boundary_flags`` is 0 when no graph was supplied, and 0 must mean *no terminus here*, so both flanks
...
def test_a_SPLICE_SITE_alone_breaks_no_population_here():
    ...
    sj channel was measured *better* under capture and *worse* off it (`ISSUES: the-cancelling-pair`)."""
    from rigel.calibration.splice_graph import (
        FLAG_ACCEPTOR_NEG,
        FLAG_ACCEPTOR_POS,
        FLAG_DONOR_NEG,
...
def test_the_two_masks_are_independent_and_an_boundary_can_carry_BOTH():
    """⚠ They are not complements. A transcript can end where another begins, so one BOUNDARY can break the
...
def _flagged_chain(flags_by_boundary):
    ...
    parts = make_chain_parts(
        ...
        boundary_pos=[20.0, 60.0, 60.0, 60.0, 60.0, 20.0],
        boundary_neg=[20.0, 10.0, 10.0, 10.0, 10.0, 20.0],
    )
    parts.statics = build_region_statics(
        parts.chain, parts.region_arrays, np.asarray(flags_by_boundary, np.uint16)
    )
    return parts
...
#: chain N E N E N E N E N E N E N — slot 2k is region k, slot 2k+1 is boundary k.
#: The six BOUNDARIES are slots 1,3,5,7,9,11; the `nested_exons` flag pattern puts a genomic LOW end on the
...
def test_a_terminus_UNLICENSES_the_step_into_the_flank_that_gains_rna():
    """⭐⭐⭐ **THE CHANGE, as a per-step statement.** At an BOUNDARY where a transcript's genomic LOW end
    sits, the step between that BOUNDARY and its RIGHT flank crosses a population difference, so the
    composition may not be imputed across it and the gDNA level must cross UNSCALED (``r_g == 1``). At an
    BOUNDARY carrying a genomic HIGH end, the same for its LEFT flank. Both directions of each pair, because
    ...
    conjunct being a blanket. On this chain each BOUNDARY breaks exactly one of its two sides, so every BOUNDARY
    contributes one broken pair and one intact pair."""
    licence = {(h["dst"], h["src"]): h for h in _hop_licence(_flagged_chain(_NESTED_FLAGS))}
    # slot 2k = region k, slot 2k+1 = boundary k
    broken, intact = [], []
    for e, bits in enumerate(_NESTED_FLAGS):
        boundary = 2 * e + 1
        gains_right = bits == FLAG_TSS_POS
        far = boundary + 1 if gains_right else boundary - 1
        near = boundary - 1 if gains_right else boundary + 1
        broken += [(boundary, far), (far, boundary)]
        intact += [(boundary, near), (near, boundary)]
...
def _ambig_chain(flags_by_boundary):
    """``intergenic | exon+ | exon AMBIG | exon+ | intergenic``, STRANDED, flags injected per BOUNDARY.
...
def test_the_RELAY_honours_the_population_test_TOO():
    ...
    BOUNDARY at slot 3, so the step from that BOUNDARY into the AMBIG region at slot 4 crosses a population
    ...
    assert lvl_f[dst] == lvl_f[boundary], (
        f"the relay scaled the level {lvl_f[boundary]:.9g} → {lvl_f[dst]:.9g} across a step that gains RNA "
        "on the destination side, so `_relay` is not applying the population test"
    )
    assert lvl_p[dst] != lvl_p[boundary], (
