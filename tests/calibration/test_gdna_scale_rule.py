...
# density ``gp`` folded in) is zeroed, and the SPLICE IN block then adds the sj COUNT'S precision back onto
...
# SPLICE IN path its fixture (no sj) never exercised. Measured at 56584 → 56583: ``n = 0 @ pn = 280``.
#
# The rule stands as landed — THIS strand changed ⇒ NO CLAIM — and the SPLICE IN is a claim about this strand.
...
# evidence. So a refused arm delivers NOTHING, SPLICE IN included: value AND precision zero, both twins.


def _splice_in_transport_fixture(pop_p: bool):
    """One hop through `_transport` where every even slot is an EXON and every odd slot a BOUNDARY
    carrying + sj flux, so the hop odd → even is a SPLICE IN; the + arm's licence is the argument."""
    import dataclasses
    from types import SimpleNamespace
    ...
    splice_in = valid & ~is_bnd & is_bnd[src]
    assert splice_in.any(), "the fixture built no SPLICE IN hop"
    return splice_in, out


def test_the_splice_in_delivers_no_precision_on_a_refused_arm_VECTORISED():
    """⛔ At a no-claim hop the SPLICE IN's sj precision must go with the arm's value — a zero at the sj
    count's precision is 'there is no + RNA here', which the source cannot claim."""
    splice_in, out = _splice_in_transport_fixture(pop_p=False)
    tp, tpp, tmp = out[1], out[4], out[7]
    assert np.all(tp[splice_in] == 0.0), "a refused + arm delivered a value"
    assert np.all(tpp[splice_in] == 0.0), (
        f"the splice_in added MODE-FUSION precision back onto a refused arm: {tpp[splice_in]}"
    )
    assert np.all(tmp[splice_in] == 0.0), (
        f"the splice_in added MEASUREMENT precision back onto a refused arm: {tmp[splice_in]}"
    )
    # …and the control: with the arm licensed the SAME fixture's SPLICE IN is live, so the gate above is not
    # passing vacuously (TRAPS: could-the-arm-have-fired).
    splice_in, out = _splice_in_transport_fixture(pop_p=True)
    assert np.all(out[4][splice_in] > 0.0) and np.all(out[7][splice_in] > 0.0), (
        "the control's SPLICE IN is dead"
    )


def _splice_in_scan_fixture(refuse: bool):
