"""The ψ single-λ + θ message interface (`simplex_logodds`, the the-single-lambda-combine rank-1 fix substrate).
...
def _regions():
    # 3 single-strand (+) regions + 1 AMBIG region, all with data, UNSTRANDED (κ=½ ⇒ the strand is flat, so the
...
def test_lambda_message_pulls_f_g_to_sigma_lambda():
    """A high-precision λ-message pulls each region's f_g to ``σ(λ_target)`` — the direct-on-λ constraint."""
    u_pos, u_neg, ap, an, mu, ms = _regions()
...
def test_lambda_message_zero_precision_is_a_noop():
    ...
    u_pos, u_neg, ap, an, mu, ms = _regions()
    dc0 = _solve_regions_logodds_all(u_pos, u_neg, ap, an, mu, ms, **_BASE)
    dc1 = _solve_regions_logodds_all(
        u_pos,
        u_neg,
        ap,
        an,
...
def test_theta_message_tilts_ambig_region():
    ...
    dc_flat = _solve_regions_logodds_all(u_pos, u_neg, ap, an, mu, ms, **common)
    dc_tilt = _solve_regions_logodds_all(
        u_pos,
        u_neg,
        ap,
        an,
