"""⭐⭐⭐ THE RELAY'S MASS RESCALE IS GATED BY THE COMPOSITION-IMPUTATION LICENCE.

The relay used to restore ``Sum_c rho_c·E_c = M`` at **every** slot, filling components the message did
NOT supply from the **destination's own** density (``messages.relay``'s scan kernel, the ``_k`` block; the scalar
twin of ``_rescale_v``). For a gDNA-only message that rescale had an exact closed form in two published
quantities::
...
exactly half. So the rescale reserved half the mass budget for RNA the message never claimed, the delivered
...
``1 - R_own`` = 1/2 (attracting), so **the rescale pushed every message's composition toward the
...
fragments the destination observed. The rescale restores it with ``k = M/S``, and ``S`` fills every component
...
destination's CONSTANTS and its OBSERVATIONS and never its BELIEFS, so the rescale is licensed in exactly the
two states where nothing believed reaches ``S``:
    ...
    (ii) the destination is a structurally pure-gDNA object — there is no unsupplied component to fill in.
         ``f_g = 1`` there is STRUCTURE, so ``S = rho_g·E_g`` and the rescale hands the object its own MEASURED
         density ``M/E_g``.

Anywhere else, the identity is not implied and there is nothing for the rescale to restore.
...
built to do the same job measured inert (`region_geometry`'s deleted-landscape note). ⛔ And deleting the rescale
...
**0.2618**. Licensing the rescale takes it to **0.2264**, and the whole 36-condition gDNA ladder moves
0.0415 → 0.0414 with **no condition regressing**.

⚠⚠ **AND THE REST OF THE WAY IS BLOCKED BY TRAPS: pure-and-length-censored/TRAPS: a-threshold-on-a-fitted-residue, NOT BY THIS.** On the same toy the rescale still fires
through the gene interior, because ``may_share_composition`` is a PRECISION test with no floor and κ is FITTED at 0.500689
...
measured consequence and neither is decoration:
    ...
    ``if _may_share``                  (drop the structural branch)   3: test_gdna_scale_rule's capture gate at
                                                                   20x and 200x, + the own-measurement gate here
    ``if True``                   (the pre-fix behaviour)        7: every effect gate in this file
    ``if False``                  (delete the rescale)               5: the union of the two above
    ``_struct`` for ``g1_locked`` (region-only, so not the BOUNDARY)   3: as ``if _may_share``
...
def _uniform_field_chain(*, rho=1.0, rna, bp=1000.0, rho_first=None):
    ...
    there is no sj anywhere, so no slot ever earns RNA precision of its own and no source can may_share_composition
...
def _stranded_chain():
    ...
    can therefore may_share_composition one, while the intergenic flanks and the gene-boundary BOUNDARIES cannot. So the same
    chain carries licensed and unlicensed steps, which is what makes "the rescale fires iff licensed"
...
def _relay_walk(parts, *, kappa=0.5, n_obs=0.0):
    ...
    * ``may_share_composition`` — the composition-imputation licence for this step, recomputed here from the relay's own
      running precisions at the source (``fwd_pg``/``fwd_pp``/``fwd_pn``), which is the state ``_may_share``
    ...
    * ``g1`` — is the DESTINATION a structurally pure-gDNA object? Case (ii) of the rescale's licence.
    * ``pinned`` — the rescale's licence for this step, ``may_share_composition or g1``: the two states in which no belief
      reaches the budget.
    * ``S_over_M`` — the destination's mass identity as the relay left it. 1 means the rescale fired.
    """
    cap = {}
    final = region_sweep(
        parts.chain,
        ...
        out.append(
            {
                ...
                "may_share_composition": may_share_composition,
                "g1": bool(g1[s]),
                "pinned": (bool(may_share_composition or g1[s]) if has_src else None),
...
def test_an_unpinned_step_does_not_rescale_the_level_at_all():
    ...
    so ``Sum_c rho_c·E_c = M`` is not implied and the rescale has nothing to restore: the delivered gDNA
    ...
    ⛔ Pre-fix this failed at every step: the rescale rescaled by ``1/(phi_msg + R_own)``, which on this
    ...
    for row in steps:
        assert row["level_out"] == row["level_in"], (
            ...
            f"(k = {row['level_out'] / row['level_in']:.9f}; the rescale's old closed form gives "
            f"{1.0 / (row['phi_msg'] + row['R_own']):.9f})"
        )


def test_the_pin_fires_ONLY_where_no_belief_can_reach_its_budget():
    """⭐⭐⭐ **THIS IS A LICENCE AND NOT A DELETION, and this gate is the whole difference.** The rescale's
    ...
    * **(ii) the destination is structurally pure gDNA.** There is no unsupplied component to fill in:
      ``f_g = 1`` is STRUCTURE, so the budget is ``rho_g·E_g`` and the rescale hands the object its own
    ...
    * **not licensed anywhere else on this chain** — unstranded, no sj, so no slot earns RNA
      precision and nothing can may_share_composition a composition. Every other step must leave the level alone.
    ...
    assert all(r["g1"] and not r["may_share_composition"] for r in pin), (
    ...
    for row in pin:
        assert row["level_out"] == pytest.approx(row["M_over_Eg"], rel=1e-12), (
            f"slot {row['slot']} is structurally pure gDNA, so the rescale must hand it its own measured "
            f"density {row['M_over_Eg']:.9g}; it delivered {row['level_out']:.9g}"
        )
        assert row["S_over_M"] == pytest.approx(1.0, rel=1e-12), (
            f"slot {row['slot']}: the mass identity reads {row['S_over_M']:.9f} after the rescale"
        )
    for row in off:
        assert row["level_out"] == row["level_in"], (
        ...
        assert abs(row["S_over_M"] - 1.0) > 0.05, (
            f"slot {row['slot']}: the identity happens to hold ({row['S_over_M']:.6f}) with the rescale "
            "off, so the unpinned arm of this gate proves nothing"
        )


def test_the_lend_branch_of_the_licence_is_live():
    """⭐ **CASE (i) IS REACHABLE AND IS NOT SHADOWED BY CASE (ii).** The rescale's licence is
    ``may_share_composition or g1``, and a gate set that only ever exercises the ``g1`` branch would pass with ``may_share_composition``
    ...
    The fixture is STRANDED, so the genic regions earn composition evidence of their own and can may_share_composition one,
...
def test_an_evidence_free_object_still_reserves_half_a_budget_that_no_longer_rescales_anything():
    ...
    free RNA strand, so ``R_own`` is 0.5000 to machine precision and the rescale's old factor
    ...
    half** — the fix changed the rescale's licence, not the reference. What it also asserts is that the half
...
def test_the_running_product_of_the_rescales_is_exactly_one_at_every_step():
    ...
    the rescale licensed, every step on this chain has a rescale of exactly 1, so that product is 1 at every
...
def test_the_delivered_level_tracks_the_gdna_FIELD_and_not_any_destinations_own_total():
    """⛔⛔ **TRAPS: a-message-from-the-destinations-belief, AS AN INVARIANCE, AT THE DESTINATIONS WHERE TRAPS: a-message-from-the-destinations-belief ACTUALLY BIT.** The old rescale's fixed point
...
def test_a_structurally_pure_gdna_destination_IS_told_its_own_measurement():
    ...
    is the latter. So the rescale fires here by case (ii) of its licence and overwrites the relayed level —
    ...
    for rho_first in (1.0, 10.0):
        ...
        row = walk[BOUNDARY]
        assert row["g1"] and not row["may_share_composition"], (
            f"slot {BOUNDARY} is not the structural case (g1={row['g1']}, may_share_composition={row['may_share_composition']})"
        )
...
@pytest.mark.xfail(
    reason=(
        "NOT the nested-exons probe, and not the mass rescale: the level reaching the RNA-free interior exon is now the field "
