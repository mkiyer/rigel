...
def test_G2_psi_slope_in_the_vertex_tail_is_exactly_minus_the_reference_exponent():
    ...
    computes (`TRAPS.md` A1)."""
    lam, fg = _logodds_grid(1024, 10.0)
    u_pos, u_neg, ap, an, _mu, _ms = _nodes(1)
    psi, _fp, _fn = SL._local_loglik_logodds(
        u_pos, u_neg, ap, an, 0.5, 0.0, 0.0, lam, fg,
        np.full(1, 0.5), np.full(1, 0.5), np.zeros(1),
        gdna_imp_mode=np.zeros(1), gdna_imp_prec=np.full(1, 1e3),
    )
    p = psi[0]
...
def _set_exponents(monkeypatch, c_g: float, c_r: float):
    ...
    names, so patching the functions reaches BOTH the 1-D and the 2-D solver — `TRAPS.md` A10's
...
def test_G3_each_half_of_the_constant_holds_ONE_vertex_and_is_NEGLIGIBLE_at_the_other(monkeypatch):
    ...
    lattice quantisation and not orthogonality (`TRAPS.md` A5: a bit-identity gate has lied in both
...
def test_G4_softening_the_reference_moves_lambda_MONOTONICALLY_and_by_a_BOUNDED_step(monkeypatch):
    ...
    not the fix (and why ``C → 0``, which would be, makes ψ improper — `TRAPS.md` D5).
...
def test_G5_the_SAME_claim_delivered_on_lambda_is_honoured_AT_PAR():
    """⭐⭐⭐ THE CONTROL THAT MAKES G1 A FALSIFICATION (`TRAPS.md` A3).
...
def test_G6_psi_is_BLIND_to_the_certified_RNA_channel():
    """⛔⛔ THE ROOT CAUSE, AS AN OBSERVABLE — and the one a fix MUST move (`TRAPS.md` B14).
    ...
    **bit-identical**. ⚠ That exclusion is deliberate and its stated reason is sound as far as it goes —
    ...
    Gate ``C2b`` pins that. The information in this channel lives entirely in ``S > 0`` and points AWAY
    ...
    on 12 of the 36 ladder conditions (gates ``C2``/``C3``). What survives is the algebra, not the
    coefficient: reference + term is exactly Beta(½, ½+S), gate ``C4``."""
