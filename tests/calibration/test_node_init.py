...
One test per information source of `docs/calibration/variance_model_concepts.md` — MEASURED (Poisson
...
def test_tau_factory_isolates_the_density_deconvolution_from_the_strand():
    """``tau_factory`` must carry the DENSITY-deconvolution half of ``tau_lam`` alone, on the same nodes.

    The two halves are not interchangeable: ``I_strand`` is a composition VOTE, while the factory is a direct
    DENSITY measurement of this node's own composition — and `bp_solver` uses the latter to refuse an imputed
    λ message on the axis the node has already measured. Mixing the strand in would silence λ messages at
    every stranded node, which is a different (and unmeasured) change."""
    chain, statics, geometry, belief, region_arrays = _scenario(kappa=0.5)  # unstranded ⇒ strand τ = 0
    lam, _ = _logodds_grid(60, 10.0)
    prior = np.zeros((chain.n_nodes, lam.shape[0]))
    prior[5] = -0.5 * ((lam - 2.0) ** 2) / 0.05
    common = dict(
        kappa=0.5, od_g=0.2, od_r=0.1, n_gdna_obs=230.0, n_rna_obs=85.0,
        n_grid=60, logodds_window=10.0, n_tilt=None, n_grid_ss=256, belief=belief,
    )
    ni_off = build_node_init(chain, statics, geometry, **common)
    ni_on = build_node_init(chain, statics, geometry, intron_prior=prior, **common)
    # no factory ⇒ nothing is claimed anywhere
    assert np.all(ni_off.tau_factory == 0.0)
    assert ni_off.tau_factory.shape == ni_off.tau_lam.shape
    # with a factory ⇒ exactly the factored node claims, and on an unstranded chain it IS the whole τ
    assert ni_on.tau_factory[5] > 0.0
    assert np.all(ni_on.tau_factory[np.arange(chain.n_nodes) != 5] == 0.0)
    assert abs(ni_on.tau_factory[5] - ni_on.tau_lam[5]) <= 1e-12 * abs(ni_on.tau_lam[5])


def test_tau_factory_excludes_the_strand_term_when_stranded():
    """On a STRANDED chain ``tau_lam`` gains the strand λ-term while ``tau_factory`` must not — otherwise the
    solver's "a direct measurement outranks an imputation" rule would fire on strand-solved nodes too."""
    chain, statics, geometry, belief, region_arrays = _scenario(kappa=0.95)
    lam, _ = _logodds_grid(60, 10.0)
    prior = np.zeros((chain.n_nodes, lam.shape[0]))
    prior[5] = -0.5 * ((lam - 2.0) ** 2) / 0.05
    ni = build_node_init(
        chain, statics, geometry, kappa=0.95, od_g=0.2, od_r=0.1, n_gdna_obs=230.0,
        n_rna_obs=85.0, n_grid=60, logodds_window=10.0, n_tilt=None, n_grid_ss=256,
        belief=belief, intron_prior=prior,
    )
    single = np.asarray(statics.free_pos, bool) ^ np.asarray(statics.free_neg, bool)
    strand_only = single & (np.arange(chain.n_nodes) != 5)
    assert np.any(ni.tau_lam[strand_only] > 0.0)  # the strand does inform those nodes …
    assert np.all(ni.tau_factory[strand_only] == 0.0)  # … but that is NOT a density measurement
