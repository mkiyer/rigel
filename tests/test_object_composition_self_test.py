"""``object_composition.py --self-test`` is a REGRESSION GATE, not a thing to remember to run.

⭐ The instrument carries its own falsification — every comparator perturbed and required to FIRE, with
no I/O — which is the project's standing rule that writing a gate is only half of it. But a
...
what proves the ``_sibling`` loader still finds ``prior_vs_oracle``.
...
SCRIPT = ROOT / "scripts" / "design" / "object_composition.py"
...
def _load():
    ...
    spec = importlib.util.spec_from_file_location("object_composition", SCRIPT)
    module = importlib.util.module_from_spec(spec)
    sys.modules["object_composition"] = module
...
def test_every_self_test_perturbation_still_fires():
    ...
    with redirect_stdout(buf):
        rc = module.self_test()
    out = buf.getvalue()
    assert rc == 0, f"object_composition --self-test FAILED:\n{out}"
    assert f"{EXPECTED_GATES}/{EXPECTED_GATES} gates pass" in out, (
