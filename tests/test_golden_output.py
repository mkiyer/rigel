...
def _compare_df(actual, expected, numeric_cols, label):
    ...
    for col in numeric_cols:
        ...
        nan_mask = np.isnan(e)
        if nan_mask.any():
            assert np.isnan(a[nan_mask]).all(), f"{label}.{col}: NaN mismatch"
            a = a[~nan_mask]
            e = e[~nan_mask]
...
def _compare_output(actual, expected, scenario_name):
    ...
    # Transcript-level
    _compare_df(
        actual["transcript_df"],
        expected["transcript_df"],
        _TRANSCRIPT_NUMERIC_COLS,
        f"{scenario_name}/transcript",
    )
...
def _run_golden_test(scenario_name, result, index, update_golden):
    ...
    if not _golden_exists(scenario_name):
        _save_golden(scenario_name, output)
        pytest.skip(
            f"Golden files created for '{scenario_name}' (first run). Re-run to verify stability."
        )
...
class TestSingleGene:
    ...
    def test_multi_exon_spliced(self, tmp_path, update_golden):
        sc = Scenario("golden_3e", genome_length=8000, seed=SEED, work_dir=tmp_path / "golden_3e")
        sc.add_gene(
            "g1",
            "+",
            [
                {"t_id": "t1", "exons": [(200, 500), (1000, 1300), (2000, 2300)], "abundance": 80},
            ],
        )
        sc.add_gene(
            "g_h",
...
class TestMultiIsoform:
    ...
    def test_two_isoforms_equal(self, tmp_path, update_golden):
        sc = Scenario(
            "golden_2iso_eq", genome_length=8000, seed=SEED, work_dir=tmp_path / "golden_2iso_eq"
        )
        sc.add_gene(
            "g1",
            "+",
            [
                {"t_id": "t1", "exons": [(500, 800), (1200, 1500)], "abundance": 50},
                {"t_id": "t2", "exons": [(500, 800), (1800, 2100)], "abundance": 50},
            ],
        )
        sc.add_gene(
            "g_h",
    ...
    def test_three_isoforms_skewed(self, tmp_path, update_golden):
        ...
        sc = Scenario(
            "golden_3iso", genome_length=10000, seed=SEED, work_dir=tmp_path / "golden_3iso"
        )
        sc.add_gene(
            "g1",
            "+",
            [
                {"t_id": "t1", "exons": [(500, 800), (1200, 1400), (2000, 2300)], "abundance": 100},
                {"t_id": "t2", "exons": [(500, 800), (1600, 1800), (2000, 2300)], "abundance": 30},
                {"t_id": "t3", "exons": [(500, 800), (2000, 2300)], "abundance": 10},
            ],
        )
        sc.add_gene(
            "g_h",
    ...
    def test_five_isoforms(self, tmp_path, update_golden):
        ...
        sc = Scenario(
            "golden_5iso", genome_length=12000, seed=SEED, work_dir=tmp_path / "golden_5iso"
        )
        isoforms = []
        for i in range(5):
            mid = 1200 + i * 400
...
class TestAntisense:
    ...
    def test_antisense_overlap(self, tmp_path, update_golden):
        sc = Scenario("golden_as", genome_length=8000, seed=SEED, work_dir=tmp_path / "golden_as")
        sc.add_gene(
            "g_sense",
            "+",
            [
                {"t_id": "t_sense", "exons": [(500, 800), (1200, 1500)], "abundance": 60},
            ],
        )
        sc.add_gene(
            "g_anti",
            "-",
            [
                {"t_id": "t_anti", "exons": [(400, 700), (1100, 1400)], "abundance": 40},
            ],
        )
...
class TestGDNA:
    ...
    def test_gdna_light(self, tmp_path, update_golden):
        gdna = GDNAConfig(abundance=10.0, frag_mean=350, frag_std=100, frag_min=100, frag_max=1000)
        sc = Scenario(
            "golden_gdna_lo", genome_length=10000, seed=SEED, work_dir=tmp_path / "golden_gdna_lo"
        )
        sc.add_gene(
            "g1",
            "+",
            [
                {"t_id": "t1", "exons": [(500, 800), (1200, 1500)], "abundance": 60},
            ],
        )
        sc.add_gene(
            "g2",
            "-",
            [
                {"t_id": "t2", "exons": [(4000, 4300), (4700, 5000)], "abundance": 40},
            ],
        )
        result = sc.build_oracle(n_fragments=N_FRAGS, sim_config=_SIM_SS100, gdna_config=gdna)
        _run_golden_test("gdna_light", result, result.index, update_golden)
        sc.cleanup()

    def test_gdna_heavy(self, tmp_path, update_golden):
        gdna = GDNAConfig(abundance=80.0, frag_mean=350, frag_std=100, frag_min=100, frag_max=1000)
        sc = Scenario(
            "golden_gdna_hi", genome_length=10000, seed=SEED, work_dir=tmp_path / "golden_gdna_hi"
        )
        sc.add_gene(
            "g1",
            "+",
            [
                {"t_id": "t1", "exons": [(500, 800), (1200, 1500)], "abundance": 40},
            ],
        )
        sc.add_gene(
            "g2",
            "-",
            [
                {"t_id": "t2", "exons": [(4000, 4300), (4700, 5000)], "abundance": 30},
            ],
        )
        result = sc.build_oracle(n_fragments=N_FRAGS, sim_config=_SIM_SS100, gdna_config=gdna)
...
class TestNRNA:
    ...
    def test_nrna_moderate(self, tmp_path, update_golden):
        sc = Scenario(
            "golden_nrna", genome_length=10000, seed=SEED, work_dir=tmp_path / "golden_nrna"
        )
        sc.add_gene(
            "g1",
            "+",
        ...
        result = sc.build_oracle(n_fragments=N_FRAGS, sim_config=_SIM_SS90, nrna_abundance=30)
        _run_golden_test("nrna_moderate_ss90", result, result.index, update_golden)
        sc.cleanup()

    def test_nrna_heavy(self, tmp_path, update_golden):
        sc = Scenario(
            "golden_nrna_hi", genome_length=10000, seed=SEED, work_dir=tmp_path / "golden_nrna_hi"
        )
        sc.add_gene(
            "g1",
            "+",
        ...
        result = sc.build_oracle(n_fragments=N_FRAGS, sim_config=_SIM_SS90, nrna_abundance=70)
...
class TestStrandSpecificity:
    ...
    def test_low_strand_specificity(self, tmp_path, update_golden):
        sc = Scenario(
            "golden_ss65", genome_length=10000, seed=SEED, work_dir=tmp_path / "golden_ss65"
        )
        sc.add_gene(
            "g1",
            "+",
            [
                {"t_id": "t1", "exons": [(500, 800), (1200, 1500)], "abundance": 50},
                {"t_id": "t2", "exons": [(500, 800), (1800, 2100)], "abundance": 30},
            ],
        )
        sc.add_gene(
            "g2",
            "-",
            [
                {"t_id": "t3", "exons": [(4000, 4300), (4700, 5000)], "abundance": 40},
            ],
        )
...
class TestManyLoci:
    ...
    def test_ten_genes(self, tmp_path, update_golden):
        sc = Scenario(
            "golden_10g", genome_length=60000, seed=SEED, work_dir=tmp_path / "golden_10g"
        )
        for i in range(10):
            base = 1000 + i * 5000
            ...
            if i % 3 == 0:
                ...
                exons = [(base, base + 200), (base + 500, base + 700), (base + 1100, base + 1300)]
            sc.add_gene(
                f"g{i + 1}",
                strand,
                [
                    {"t_id": f"t{i + 1}", "exons": exons, "abundance": 15 + i * 8},
                ],
            )
        result = sc.build_oracle(n_fragments=N_FRAGS, sim_config=_SIM_SS100)
    ...
    def test_mixed_loci_with_isoforms(self, tmp_path, update_golden):
        ...
        sc = Scenario(
            "golden_mixed", genome_length=50000, seed=SEED, work_dir=tmp_path / "golden_mixed"
        )
        # Gene 1: single exon
        sc.add_gene(
            "g1",
        ...
        sc.add_gene(
            ...
            "-",
            [
                {"t_id": "t2a", "exons": [(6000, 6300), (6700, 7000)], "abundance": 30},
                {"t_id": "t2b", "exons": [(6000, 6300), (7400, 7700)], "abundance": 20},
            ],
        )
        ...
        sc.add_gene(
            ...
            [
                ...
                {"t_id": "t3c", "exons": [(12000, 12300), (13400, 13700)], "abundance": 10},
            ],
        )
        ...
        sc.add_gene(
            ...
            "+",
            [
                {"t_id": "t5", "exons": [(26000, 26300), (30000, 30300)], "abundance": 35},
            ],
        )
        ...
        sc.add_gene(
            ...
            "-",
            [
                {"t_id": "t6", "exons": [(38000, 38300), (38700, 39000)], "abundance": 5},
            ],
        )
        result = sc.build_oracle(n_fragments=N_FRAGS, sim_config=_SIM_SS100)
...
class TestCombinedStress:
    ...
    def test_full_combo_moderate(self, tmp_path, update_golden):
        gdna = GDNAConfig(abundance=20.0, frag_mean=350, frag_std=100, frag_min=100, frag_max=1000)
        sc = Scenario(
            "golden_combo_mod",
            genome_length=12000,
            seed=SEED,
        ...
        sc.add_gene(
            ...
            [
                {"t_id": "t1", "exons": [(500, 800), (1200, 1500)], "abundance": 50},
                {"t_id": "t2", "exons": [(500, 800), (1800, 2100)], "abundance": 25},
            ],
        )
        sc.add_gene(
            "g2",
            "-",
            [
                {"t_id": "t3", "exons": [(5000, 5300), (5700, 6000)], "abundance": 40},
            ],
        )
        result = sc.build_oracle(
            n_fragments=N_FRAGS, sim_config=_SIM_SS90, gdna_config=gdna, nrna_abundance=20
        )
    ...
    def test_full_combo_extreme(self, tmp_path, update_golden):
        ...
        sc.add_gene(
            ...
            "+",
            [
                {"t_id": "t1", "exons": [(500, 800), (1200, 1400), (2000, 2300)], "abundance": 60},
                {"t_id": "t2", "exons": [(500, 800), (1600, 1800), (2000, 2300)], "abundance": 20},
                {"t_id": "t3", "exons": [(500, 800), (2000, 2300)], "abundance": 10},
            ],
        )
        sc.add_gene(
            ...
            "-",
            [
                {"t_id": "t4", "exons": [(5000, 5300), (5700, 6000)], "abundance": 30},
            ],
        )
        result = sc.build_oracle(
            n_fragments=N_FRAGS, sim_config=_SIM_SS65, gdna_config=gdna, nrna_abundance=60
        )
...
class TestEdgeCases:
    ...
    def test_extreme_abundance_ratio(self, tmp_path, update_golden):
        ...
        sc = Scenario(
            "golden_ratio", genome_length=8000, seed=SEED, work_dir=tmp_path / "golden_ratio"
        )
        sc.add_gene(
            "g1",
            "+",
            [
                {"t_id": "t_major", "exons": [(500, 800), (1200, 1500)], "abundance": 200},
                {"t_id": "t_minor", "exons": [(500, 800), (1800, 2100)], "abundance": 2},
            ],
        )
        sc.add_gene(
            "g_h",
    ...
    def test_zero_abundance_gene(self, tmp_path, update_golden):
        ...
        sc = Scenario(
            "golden_zero", genome_length=8000, seed=SEED, work_dir=tmp_path / "golden_zero"
        )
        sc.add_gene(
            "g1",
            "+",
        ...
        sc.add_gene(
            ...
            "-",
            [
                {"t_id": "t_zero", "exons": [(3000, 3300), (3700, 4000)], "abundance": 0},
            ],
        )
        result = sc.build_oracle(n_fragments=N_FRAGS, sim_config=_SIM_SS100)
    ...
    def test_wide_intron(self, tmp_path, update_golden):
        ...
        sc.add_gene(
            ...
            "+",
            [
                {"t_id": "t1", "exons": [(500, 800), (20000, 20300)], "abundance": 50},
            ],
        )
        sc.add_gene(
            "g_h",
    ...
    def test_higher_fragment_count(self, tmp_path, update_golden):
        ...
        sc = Scenario("golden_3k", genome_length=10000, seed=SEED, work_dir=tmp_path / "golden_3k")
        sc.add_gene(
            "g1",
            "+",
            [
