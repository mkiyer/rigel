"""Tests for two-state latent expression tensor helpers."""
...
def test_state_log_prior_favors_unexpressed_for_intergenic_and_intronic_regions() -> None:
    ...
    assert prior[0, STATE_UNEXPRESSED] == pytest.approx(2.0)
    assert prior[0, STATE_EXPRESSED] == pytest.approx(-2.0)
    assert prior[1, STATE_UNEXPRESSED] == pytest.approx(0.75)
    assert prior[1, STATE_EXPRESSED] == pytest.approx(-1.0)
    np.testing.assert_allclose(prior[2], 0.0)

    late_prior = build_state_log_prior(region_arrays, background, pass_index=2)
    assert late_prior[0, STATE_UNEXPRESSED] == pytest.approx(1.0)
...
def test_likelihood_terms_have_expected_direction() -> None:
    ...
    strand_expression = build_logbf_expression(observation, _strand_channels([0.0, 5.0]))

    assert expression[1, STATE_UNEXPRESSED] < expression[1, STATE_EXPRESSED]
    assert strand_expression[1, STATE_UNEXPRESSED] < strand_expression[1, STATE_EXPRESSED]
...
def test_strand_summary_penalizes_not_expressed_states_when_rna_lower_bound_exists() -> None:
    ...
    assert logbf[0, STATE_UNEXPRESSED] == pytest.approx(0.0)
    assert logbf[1, STATE_UNEXPRESSED] < 0.0
    assert logbf[1, STATE_EXPRESSED] == pytest.approx(0.0)
...
def test_state_tensor_rows_sum_to_one_and_identify_combined_evidence() -> None:
    ...
    p_states = build_state_tensor(state_log_prior, expression, strand)

    np.testing.assert_allclose(p_states.sum(axis=1), 1.0, rtol=1.0e-6, atol=1.0e-6)
    assert int(np.argmax(p_states[1])) == STATE_EXPRESSED
