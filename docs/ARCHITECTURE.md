...
| `bp_solver.py` | the BP solver: the single forward-backward `node_sweep` (gDNA + per-strand RNA messages) over the derived message-variance model (laws M1–M11, `docs/calibration/variance_ledger.md`) + the region / boundary-side projections (`chain_region_deconv` / `chain_boundary_side_deconv`) |
