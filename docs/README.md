...
| [`message_propagation_arithmetic.md`](calibration/message_propagation_arithmetic.md) | **★★ THE mode derivation (2026-07-21, MC-validated)** — the rigorous, Monte-Carlo–validated message **MODE** arithmetic on the `intron ⟷ boundary ⟷ exon` chain: **density is the frame-invariant currency** (Theorem 1); the capture **cliff** and the **mature** change occupy **different edges** (Theorem 2 — FL-frame shift on `intron⟷boundary`, spliced add/subtract on `boundary⟷exon`); the `exon→boundary` **absorption** ("hardest node") solved; the "gDNA-always" question answered (density transfers, fraction needs the RNA normalizer — Theorem 4). Validated to |Δf_g| < 0.003. Precision + solve-gating deferred. Harness: `scripts/debug/chain_mode_mc.py`. |
