...
| [`propagation_implementation_plan.md`](calibration/propagation_implementation_plan.md) | **`propagation.py` SPEC** (validated by both prototypes): node graph, per-node 3-term solve, seeds, precision-weighted enrichment-matched propagation, seed→sweep→converge loop, the consolidation map, and **§11 the remaining issues to settle next** |
