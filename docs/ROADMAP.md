...
|---|---|---|
| **1** | ⭐⭐⭐ **FIX THE RELAY'S COMPOSITION LICENCE — the ONE named bug that gates everything below it** | `HeadPolicy`'s licence implements *"where two objects do not measure the same population, the composition may not be imputed"* for transcript TERMINI only (`terminus_flank_gain`). **`mrna_active` flipping is a population change too and is NOT checked**, so an intron's correct pure-gDNA claim is relayed into the adjacent exon and drives it to a confident wrong vertex. The predicate is the solver's own; the licence simply does not consult it. | ⛔⛔ **EVERY RELAY NUMBER ON RECORD WAS MEASURED WITH THIS BUG LIVE**, including the +154.8 % that keeps `message_propagation` off — and on a panel retired 2026-08-13. ⭐ Strict xfail already waiting in `tests/calibration/test_structural_reference.py`; it must go green BY THE CODE BEING FIXED, never by widening a bound. ⚠ Check §4.1's `zc_struct_lock_g1` row first — this may be half of `TRAPS: a-cancelling-defect-pair` |
| **2** | ⭐⭐⭐ **RE-PRICE `message_propagation = True` WITH THE LICENCE FIXED** | `ladder_arm_ab.py --arm base --messages off` vs `--messages on`, `--jobs 8`, all 16, per stratum, both zero controls. ⛔ The owner's 2026-08-10 ruling is NOT re-opened by a measurement — but the number behind it is stale in two independent ways (the bug, and the retired panel). **RE-PRICE, NEVER INHERIT** | ⚠ The recorded price on zero-gDNA golden scenarios was 0.029 → 89.93 and 0.005 → 9.58. If that survives the licence fix, the relay is still not shippable and rank 3 must carry the exon alone |
...
is 3,498 passed / 10 xfail / 0 failed.** The history below is kept because it is the evidence for the new
