...
| **1b** | ⭐⭐⭐ **THE gDNA FRAGMENT-LENGTH MODEL IS FITTED FROM POOLS THAT ARE NOT PURE — diagnosed 2026-08-30, three independent defects, and the fix is designed but unbuilt.** `fl.py` asserts "Every pool is PURE BY CONSTRUCTION"; measured against the origin-split oracle the intronic pool is **95 % nascent** and the intergenic pool **53 % mature** RNA (unannotated transcription reads as intergenic). ⭐ The bias is `RNA_share × (len_RNA − len_gDNA)`, which is why no panel caught it: the ladder and test chromosome give both components EQUAL lengths deliberately, so a 95 %-contaminated pool reads −0.8 bp there and −121 bp on the fl-gap arm. ⚠ It reaches the EM as a per-fragment length likelihood, not only as an opportunity input. **The gate that decides it:** the price is UNRANKABLE until the fl-gap arms are regenerated on the current sparse-nascent model — `length_ceiling.py`'s −5.90 % ceiling is "all capture-ON", which belongs to the OTHER two defects (a probe-blind opportunity divisor, and an EB shrinkage anchored on the global MIXTURE at a magic ESS). ⛔ Template subtraction is already REFUTED by measurement. Instruments: `fl_pool_purity.py`, `length_ceiling.py`, `fl_anchor_gap.py` |
