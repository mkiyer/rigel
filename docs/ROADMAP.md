...
| **3** | ⭐⭐⭐ **THE MEASURED PRIOR — the reference is FITTED from composition-free observables BEFORE pass-0, and the accumulator half LANDED 2026-08-21** | ⭐⭐ The owner's architecture (2026-08-21): "prior-free" is retired as a goal — ψ needs a proper prior, and the correct one is FITTED, not conjured. TOTAL abundance is measurable per slot with no composition model (BOUNDARY: the exact reciprocal banks + certified populations at their own divisor; REGION: the new `region_start_count`/`region_end_count` over `ℓ` — opportunity `ℓ` for EVERY fragment length, wall-blind only at opposite template ends, side-selected). A Poisson-kernel landscape fitted on those totals pre-pass-0 (no circularity — its inputs are counts and lengths) is bimodal under capture and supplies per slot the enrichment state and the span `R` = the MODE RATIO — never the 2.6–3.6×-under-reading anchors. The pass-0 gDNA prior is the two-atom mixture `{rho_0, rho_0·R}` capped by the slot's own total, per-slot weight from its own mode responsibility, delivered as a density on the λ grid (`_location_term`'s door): the POSTERIOR picks; silent evidence stays wide. ⭐ The START/END/SPAN banks are LANDED and both panels re-certified 16/16 + 16/16; the calibration half is next, VALIDATE-FIRST (the owner's sequencing): `total_abundance` against certified truth on all 32 before anything consumes it | ⛔⛔ **Every cheap alternative is now measured-refused on the current tree**: the library-wide truth mean (2.65× at `g05` stranded × ON; a cancelling pair inside a 1.001× total at `g98 ss0.50 off`), and BOTH single-mode per-object locations — `pooled` AND the flank-cancellation `local` — at 5–8× on stranded × ON with identical `B exon\|exon` damage (a near-vertex location is a strong claim in nats whatever the strength budget). ⚠ The equal-length panels are a NULL for the composition half of the abundance fix BY DESIGN; the truncation and wall halves are visible, and the fl-gap SIDE panel (never a ladder rung) is what fully exerts it. ⛔ The small-exon length-composition channel is DEFERRED (owner, 2026-08-21), with the 0.8.0 retirement; `region_span_count` stays dark behind its ledger invariants |
