...
| ⭐⭐⭐ **THE gDNA LENGTH MODEL HAS TWO ESTIMANDS AND THEY ARE ROUTED — 2026-08-31** (`FLModels.gdna_pmf` vs `gdna_realized_pmf`; gates in `tests/calibration/test_fl_realized.py`; priced by `em_fl_ceiling.py` and `quant_accuracy.py`) | "the gDNA fragment-length distribution" was ONE name over TWO quantities. **`gdna_pmf` is the UNIFORM-FRAME law** — what the chemistry makes, and what the opportunity/prior mathematics assumes, since those formulas count start positions under uniform placement and are applied where `rho` is fitted (the off-target stratum). **`gdna_realized_pmf` is the LIBRARY CENSUS** — what a sequenced gDNA fragment looks like, capture's selection included, which is what the EM's per-fragment scorer conditions on. ⭐ **Off capture they are the SAME ARRAY**, which is why one name sufficed until capture split them. ⛔⛔ **The cost of confusing them is measured, not argued: the realized law fed to GEOMETRY is +188,208 misassigned transcripts on one ladder row, while fed to the SCORER it helps** — and the fitted pmf's "bias" had been accidentally holding geometry in its own frame. ⭐ Landed: the scorer reads the realized law, every geometry consumer keeps `gdna_pmf` BIT-IDENTICALLY (gated). Direction: the +150,809 regression falls to **+293**, net **−4,350** transcripts across eleven scenarios spanning both fl-gap sign arms and the whole capture spectrum, with every capture-OFF row within ±11 and the `g00` zero control at +1. ⚠ **The RNA pmf does NOT mirror this** — its three consumers split scorer +156 / drain −822 / **geometry −18,208**, so RNA needs no second estimand, just a bias fix at `effective_lengths_em` (§2) |
