...
|---|---|---|
| **1** | ⭐⭐⭐ **THE ZERO-CONTROL GAP: ONE FEEDER CLOSED, THE OTHER NAMED, PRICED AND OPEN — the empty-slot chimera into the mass pin** | ⭐ **2026-08-18 (session 2), the dissection loop again.** The 62 k → 228 k regression at `g00 nrna_mid capture_on` walked to ONE mechanism: a zero RNA claim WITH LIVE PRECISION, relayed one hop, where the pin's licence (`_may_share`: both arms supplied) rescales the budget onto gDNA — `k = M/(tg·E_g)` = 467,000× — and delivers `M/E_g`, "all your mass is gDNA", into a 26 k-fragment exon (true 0.000 → 0.9037). TWO feeders: ① the SPLICE IN added the sj COUNT's precision back onto an arm the per-strand rule had just zeroed (measured `n = 0 @ pn = 280`) — **FIXED**, both twins, gated (`TRAPS: zero-the-precision-with-the-value`): in-scope **0.931 / 0.999 / 1.000**, `g00` **431 k → 324 k (0.753×)** with all 8 rows improving, deferred 1.000 / 0.827; ② an EMPTY exon whose mis-scoped `struct_lock` (`~solvable`, the strict xfail) owns "RNA = 0 @ 1/trigamma(½)" on every arm — the licence now isolates that arm at a terminus and the pin turns it into all-gDNA. ⛔ **The remaining bar: `g00` 324 k against the pre-licence relay's 154 k, and 193 k of it is the crime-scene condition, all of it feeder ②.** | ⭐⭐ **THREE arms priced on all 32, per stratum, and NONE lands under the written bar (`TRAPS: panel-before-src`, sixth firing) — read the mechanisms, they are the next dissection.** ⚠ every ratio below is ÷ the ①-fixed tree. `g1` (`struct_lock = g1_locked ∧ REGION`, the xfail's fix): `g00` **0.71×** (232 k) but the four `nrna_none` zero controls **1.8–15×** worse and in-scope **+2.5 / +2.1 / +0.5 %** (deferred +17 %) — the empty exons' "gDNA = 0 @ 0.2026" is load-bearing at AMBIG `exon|exon` boundaries in an RNA-only library (an RNA+ level with no RNA− claim drifts ψ to 0.38 without it). `pinM` (the pin refuses a ZERO-MASS source): `g00` **0.41× (134 k — 0.31× the licence tree, better than the pre-licence relay's 154 k)**, unstranded × OFF 0.999, stranded × OFF 1.004, ⛔ **stranded × ON 1.032, six of six worse (1.02–1.13)**, deferred 1.137 — under capture the empty slots between probe-covered stretches are the CONDUITS a relayed composition travels through (`TRAPS: the-divergence-was-a-barrier`) and the pin at those hops is load-bearing. ⭐ **So the sharp predicate is neither**: refuse the pin for a source's OWN zero-count RNA artefact and keep it for a RELAYED composition passing through an empty slot — which is what the `struct_lock` rescope does structurally, whose remaining cost is a ψ-side question at AMBIG `exon|exon` boundaries (why does an RNA+ level claim with no RNA− claim book the remainder as gDNA?). Price the pair, never a half (`TRAPS: a-cancelling-defect-pair`). The dev-sandbox handoff carries the step-by-step and the arm plumbing |
| **2** | ✅→⭐ **THE RELAY IS RE-PRICED (twice, 2026-08-18) AND ON** — what remains is closing its ~1.5–1.65× cost on the in-scope contaminated strata, bug by bug | Re-priced on the 32-condition nascent ladder against certified truth (`relay_pool_ab.py`): a large win on unstranded × capture-ON (0.31× vs muted) and every zero control, a 1.5–1.65× cost on the three in-scope contaminated strata (down from ~2.0× pre-licence). The zero-gDNA golden catastrophe (0.029 → 89.93) is GONE — those goldens now read ~0.007–0.04 with the relay on. ⚠ The 2026-08-07 mute table is history: measured with the licence bug live on a retired panel | ⭐ Candidates in the queue, measured once each at the crime scene: `transfer_var` off read 0.0010 (better than either licence fix alone — understand before shipping); `strand_population` measured INERT on this panel (annotation covers flips with TSS/TES bits) — keep or cut is an owner call. ⚠ The 1.5–1.65× cost was measured BEFORE the SPLICE IN fix (rank 1 ①), which moved unstranded × OFF 0.931× — re-price against muted before quoting it |
