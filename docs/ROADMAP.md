...
| ⭐⭐⭐ **THE FRAGMENT-LENGTH MODEL: THE PURITY ASSERTION IS GONE AND THE TWO ESTIMANDS ARE ROUTED — 2026-08-31** (`calibration/fl.py` + `gdna_density.py`; gates in `tests/calibration/test_fl_realized.py` and `test_gdna_density.py`; priced by `em_fl_ceiling.py` and `quant_accuracy.py`) | **① THE CONTAMINATION.** `fl.py` asserted "every pool is PURE BY CONSTRUCTION"; measured, the intronic pool is **95 % nascent** and the intergenic **53 % mature**. It now solves a **two-pool contrast** — the two CONTAINED pools are the same two shapes at different mixing weights, so the contaminant cancels with **no template** — with weights from a gDNA density read off the **LOW side** of the per-object density, where a contaminant that only ADDS cannot reach (the away-half again; `TRAPS: purity-is-a-property-of-the-annotation`). **② THE TWO ESTIMANDS.** "The gDNA fragment-length distribution" was ONE NAME over TWO quantities: `gdna_pmf` is the **UNIFORM-FRAME law** (the chemistry, and what the opportunity/prior mathematics assumes, since it counts start positions under uniform placement and is applied where `rho` is fitted), while `gdna_realized_pmf` is the **LIBRARY CENSUS** (capture's selection included — what the EM's per-fragment scorer conditions on). ⛔⛔ **Confusing them costs +188,208 misassigned transcripts** on one ladder row; the fitted pmf's "bias" had been accidentally holding geometry in its own frame. **③ THEY ARE COUPLED, NOT SWITCHED BETWEEN.** Capture is a spectrum and at either end one stratum has no data, so the split is weighted by how well it is RESOLVED (`lam = S/(S+N)`), and **`N = 1/m_C + 1/m_B` diverges when EITHER mass collapses** — so `lam -> 0` and the two laws become the SAME ARRAY from both ends, converging on the precision-weighted consensus. `lam = 1` is the uncoupled behaviour. ⭐ **Structural facts that will not decay**: NO constant is introduced anywhere (the density is the root of a one-sided Poisson moment with a closed form — De Moivre; every "is it big enough" test is `lam`); the contrast is the IDENTITY when the two pools agree; `g00` declines and is EXACTLY unchanged; and it needs **no accumulator or schema change**. ⭐ Direction, eleven scenarios spanning both fl-gap sign arms and the whole capture spectrum: the length pmf's error falls ~90 %, the +150,809 routing regression → **+293**, net **−7,817** transcripts, every capture-OFF row within ±11. Removing the three thresholds was itself worth **−3,467**. ⚠ The on-target excess carries its OWN weight, not `lam` — `lam` asks whether the strata's LAWS differ, the excess whether an exon's ENRICHMENT differs from 1; gating one on the other suppressed a resolved correction. ⛔ **THE STANDING RISK**: under capture the shared-contaminant premise is FALSE (TV 0.95 vs 0.06–0.14 off it) and the contrast survives only because the intergenic pool is *depleted, not impure* there, so `a_0` clips to 1 and the algebra collapses to `g = f_0` (verified to 3e-17) — **a probe panel that put RNA back into intergenic space would break it silently.** ⚠ **The RNA law does NOT mirror this** — its consumers split scorer +156 / drain −822 / **geometry −18,208** — see §2 |
...
1. ⭐⭐ **THE TWO CAPTURE-ON LENGTH DEFECTS THAT ARE *NOT* CONTAMINATION — independent of everything the
   2026-08-31 work landed, and they own the already-priced −5.90 % ceiling.**
   **(B) The gDNA opportunity divisor is blind to the probe panel.** `gdna_opportunity_from_index` is
   computed from the INDEX alone, so under capture — where the two CROSSING pools dominate and a crossing
   fragment must be long enough to span its boundary — it removes only ~6 bp of a ~30 bp length selection.
   ⭐ This is what `fl_anchor_gap.py`'s `G-gdna` control has been reporting as *"impossible, and therefore
   a bug"* (+6.0 % on all six capture-ON rows) since 2026-08-17: gDNA has no introns to miss, so a moving
   control is a divisor error. `capture_eff_length` already models the probe panel and the divisor should
   use it. ⚠ It is also what makes the crossing pools' WEIGHTS unestimable under capture (estimated
   `a_2`/`a_3` came back 0.002/0.002 against a truth of 0.980/1.000), which is what blocks a boundary-pair
   contrast.
   **(C) The EB shrinkage anchors on the global MIXTURE at a magic ESS.** `POOL_EB_PRIOR_ESS = 1000.0`
   (its own comment says *"Revisit the default on real-data pool sizes"*) shrinks the gDNA pmf toward
   `global_pmf` — every deposited fragment, a mixture that is mostly RNA whenever gDNA is a minority.
   ⚠ **Measured INERT on the ladder** (0.01 bp, pools of 1.5–3.8 M against an ESS of 1,000), and measured
   **DOMINANT on the fl-gap arm at `g05` capture-ON**, where the gDNA pools collapse (`ship−pool` = −23.7
   of a −31.7 total). So it is a real defect whose size is a function of pool depth, and the same shape as
   the deleted `Beta(14,14)`: a constant-weighted pull toward a contaminated anchor. ⭐ The derived
   replacement is the one the strand fit already uses — reconcile the pools against EACH OTHER by their own
   precision (`EQUATIONS.md` §6c), never toward a mixture.
