...
| ✅ **ψ's REFERENCE MEAN FROM THE ANNOTATION IS ON** (`structural_reference = True`; `ladder_arm_ab.py`) | `m → 0.75` wherever `¬mrna_active` and a neutral ½ elsewhere: better on **all** contaminated conditions and **BIT-IDENTICAL on all eight zero-control rows**. ⛔ The STRENGTH was the work, not the location — `strength = logit(m)`, one pseudo-observation gives 0.75 exactly, and the lattice-strength form scored better on DELIVER while being **worse than no prior at being refuted** |
...
| **1b** | ⭐⭐⭐ **JUDGE ψ's STRUCTURAL REFERENCE AS A *STRENGTH*, ON THE PANEL, WITH NO TOY — newly possible, and nearly free.** `structural_reference` asserts `m = 0.75` wherever `¬mrna_active`; the LOCATION is settled and the STRENGTH is what was never rankable. The sparse-nascent rebuild is what changes that: **DELIVER and REFUTE now coexist inside a SINGLE condition** — a large minority of live intron slots carry no nascent at all and their certified truth is exactly `f_g = 1` (the claim is TRUE there and must be delivered), beside slots whose nascent must overturn it — where the retired uniform model put nascent in essentially every intron and so scored one obligation at a time | ⭐ **Everything the measurement needs already exists**: the no-prior control is one existing config bool (`structural_reference = False`, neutral ½ everywhere); the DELIVER/REFUTE mask is FREE from the certified per-slot truth (`calibration_oracle.py`'s `slot_truth`, partitioning on whether true `f_g` is exactly 1); and the strength sweep is `vertex_ceiling.py --arm ref=A,B`, which drives ψ's two Beta pseudo-count exponents independently. ⛔⛔ **SCORE THE TWO OBLIGATIONS SEPARATELY AND NEVER POOL THEM** — a pooled score on a panel that could only DELIVER is exactly what once selected the WORST available strength, because there more nats is monotonically better (`TRAPS: a-refutability-test-needs-the-refuting-channel-in-the-fixture`). ⛔⛔ **AND READ `R intron` AND `B exon\|intron` AS SEPARATE ROWS.** The reference makes the SAME claim over the SAME population at both, and the same nascent RNA is what refutes it — but the refutation channels are NOT symmetric: the intron factory's λ-factor is **structurally zero at every BOUNDARY slot** (a boundary's count is a crossing scored against a different support, so the same NegBinom cannot be applied there), and the strand mechanism is dead at κ = ½. ⛔ So on **unstranded** data the boundary population has NO refutation channel and the prior is the only voice; pooling the two reports the region's refutability as if the boundary had it |
