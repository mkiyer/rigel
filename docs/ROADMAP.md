...
| | | |
|---|---|---|
| ⛔⛔ **THE THIRD POLICY: BEST ARM AT EVERY ZERO CONTROL ON THE REAL PANEL, AND STILL BEHIND `SilentPolicy` ON ALL THREE IN-SCOPE CONTAMINATED STRATA** | ⭐ **MEASURED 2026-08-20 on all 16 conditions**, `relay_pool_ab.py` axis ALL, gDNA absolute error in FRAGMENTS, artifact `<ladder>/benchmark/ladder_weighted_premise.tsv`. **ZERO CONTROLS — currency wins all four**, Silent / Relay / **currency**: `1,381,959 / 54,852 / **46,137**`; `416,652 / 122,976 / **15,624**` (7.9× better than the relay); `68,431 / 15,527 / **6,441**`; `31,692 / 42,378 / **3,624**` (11.7×). ⛔ **CONTAMINATED rows summed per stratum**: unstranded × capture-OFF **357,580** / 496,226 / 583,028; stranded × capture-OFF **291,815** / 440,209 / 635,702; stranded × capture-ON **564,678** / 961,174 / 848,458 (currency beats the relay here); DEFERRED unstranded × capture-ON 18,559,229 / **6,021,441** / 7,890,981. ⛔ **`SilentPolicy` still wins all three IN-SCOPE strata**: on this panel message propagation is net-harmful wherever the local solve has evidence, for BOTH message policies, and its value is concentrated where that solve is BLIND | ⭐⭐ **The weighted-premise fix moved 11 of 16 rows and up to 2.5×** (stranded × capture-ON 2,158,432 → 848,458), which is what a mechanism that was INERT on the panel looks like when it starts working — but it did not change the RANK on the in-scope strata. ⚠ **On the TEST CHROMOSOME the same instrument makes currency best on all three in-scope strata** (662 / 617 / 4,570 against Silent's 673 / 619 / 5,632): the toy and the panel disagree in RANK (`TRAPS: a-toy-and-a-panel-can-disagree-in-rank`), so **every claim about this policy must name its substrate** |
| ⭐⭐⭐ **AN ENRICHMENT RATIO IS NOW MODEL-FREE — and the quantity was already on disk, unread** | ⭐ **NEW 2026-08-20**, from the owner's question *"how are we trusting our enrichment ratios now?"* A total abundance must never be `mass / effective_length`: that divisor is a function of the composition being solved for, so 100 counts in 500 bp reads 0.25 as pure gDNA and 0.33 as pure RNA. The accumulator already deposits the RECIPROCAL OPPORTUNITY (`1/(w−1)` crossing, `1/(ℓ−w+1)` contained) — it reached `CalibrationSubstrate` and stopped. It now reaches the policy as `RegionGeometry.inv_abundance` (+ the sj bank's own column, per FACE and per TRANSCRIPT strand). `EQUATIONS.md` §3.5g | ⛔⛔ **CORRECTED 2026-08-20: the "expectation is the density EXACTLY" claim is true at a BOUNDARY (`ρ·P(w≥2) = ρ`) and FALSE at a REGION, where it is `ρ·P(w≤ℓ)` — a per-component pmf functional, 11.6× at a 98 bp exon, zero below `frag_min`** (`TRAPS: a-cancellation-is-conditional-on-its-support`; `EQUATIONS.md` §2). So every REGION↔BOUNDARY hop in the currency policy's `enrichment_ratio` carries a truncation factor with no enrichment in it. ⚠ The old caveat *"an equal-length panel is blind to this"* is TRUE only of the per-COMPONENT half; the truncation itself (`w` against `ℓ`) was measured ON the equal-length ladder's shipped banks — 0.93–1.03 agreement with `P(w≤ℓ)` on 11 of 12 live capture-OFF bins. ⛔ What is still model-dependent either way: the per-component divisors `E_g`/`E_r` |
...
|---|---|---|
| **1** | ⭐⭐⭐ **THE DISSECTION LOOP, ON THE WORST IN-SCOPE SCENARIO — the owner's protocol, 2026-08-20, and it is the method rather than one task** | ⛔ **In order, and the dissection is the step that must not be skipped**: ① run the FULL panel, all three arms, every scenario reported; ② pick the worst scenario **within the three IN-SCOPE strata** (never the deferred one, which is otherwise worst every time); ③ find the REGIONS and BOUNDARIES carrying the highest error MASS and, for a SAMPLE of them, trace the error to its root cause — from INITIALIZATION, through every belief change, every message and every refit, each step compared against the certified `slot_truth`; ④ design a fix, gated first; ⑤ **add the offending transcripts to the TEST CHROMOSOME**; ⑥ re-run the panel and see how the fix behaves everywhere, not only where it was designed; ⑦ repeat on the next highest source. ⭐⭐ **Step ⑤ is what makes the loop compound**: the test chromosome becomes a repository of the transcript combinations the tool actually fails on, and eventually stands on its own as the benchmark for comparing calibration algorithms | ⭐ **THE OPEN LEAD IS NOW MEASURED AND CLOSED AS A LEAD**: the policy's premise variance was INERT on the panel — the unweighted moment fit is dominated by the sparsest hops (median ladder slot **13** fragments, a fifth EMPTY, so `mean(v_r) = 2.07` swamped `Var(log r) = 0.76` and it floored at **0.0**, against 0.992 on the test chromosome). A PRECISION-WEIGHTED fit returns 0.294 / 0.324 on the two substrates and is landed and gated; the panel re-run moved **11 of 16 rows, up to 2.5×**, and made currency the best arm at **all four zero controls**. ⛔ **It did NOT change the rank on the in-scope contaminated strata** — `SilentPolicy` still wins all three — so the gap is NOT one mechanism, and the dissection loop above is the answer rather than another guess |
| **1a** | ⭐⭐ **CONTRAST THE THREE POLICIES — the differences are themselves a design instrument** (owner) | `SilentPolicy`, `RelayPolicy` and `CurrencyPolicy` fail DIFFERENTLY, and the production policy should be designed from what that contrast shows rather than by picking a winner. ⭐ What is measured: message propagation is net-harmful on every in-scope contaminated stratum for BOTH message policies, and its value is concentrated where the local solve is BLIND (the deferred stratum; both capture-ON zero controls, where `currency` is the best arm). ⛔ So the cheap decisive question is whether a policy that speaks **only** where the destination has no own evidence beats one that speaks everywhere — a measurement, not a switch | ⭐ The split that makes it readable is `TRAPS: an-imputation-must-cost-something-every-hop`'s: score the error separately over destinations that HAVE their own composition evidence and those that do not |
...
| **3** | ⭐⭐⭐ **THE MEASURED PRIOR — the pre-solve TOTAL abundance, and it is the road back to the calibration solve** | ⭐⭐ **THE GOAL, in one line: ψ needs a proper prior, the correct one is FITTED from composition-free observables before any solve runs, and a TOTAL abundance is the observable.** `DESIGN.md` §3.1a-i is the classification rule (four forms; a total is a PRE-solve instrument needed in exactly three places) and §3.1a-ii the wall rule; `EQUATIONS.md` §2.3b is the derivation. ⭐ **RUNG ① MEASURE — DONE, validated 32/32.** `rigel.calibration.total_abundance`: START/END over `ℓ`, side-selected by a derived wall rule. The decisive arm is FIELD-FREE (so it survives capture): both-exact regions **1.0018–1.0057**, a bound side **0.712–0.879** / **1.242–1.428** in the predicted direction, every stratum and both zero controls; truncation confirmed **1.00025/1.00045** at capture-OFF over 14.4 M fragments; model-free coverage **94.7 %** off capture, **84.3 %** on. 194 gates, 0 failures. ⭐ **RUNG ② INTEGRATE — IN PROGRESS.** ✅ `fit_intron_background` (the intergenic gDNA background → the intron λ-factor → ψ) takes the measured pair behind `CalibrationConfig.background_abundance` (`"contained"` default ⇒ bit-identical). Estimator: a TIE off capture, **1.8–4.3× better under it** (the shipped divisor's pmf is capture-distorted; a pmf-free exposure is immune). End-to-end **~1 %** — ⛔ **a FLOOR set by the panel, not a ceiling: the equal-length ladder NULLS the composition half by construction** (`E_r/E_g − 1` is +0.02…+1.6 % at gap −12.4 bp, and **−0.2…−78 %** at a realistic +87.6 bp). ✅ `background_reference` DELETED (a second implementation of that same intergenic pool — measured identical, n = 1,298 — that no caller consumed; its floor was never wired and its would-be consumer is off the solve path). ⭐⭐⭐ **THE fl-GAP SIDE PANEL IS BUILT AND MEASURED (2026-08-21), AND IT REFUTED THE COMPOSITION-HALF HYPOTHESIS FOR THESE CONSUMERS — which is the most useful thing it could have done.** Two arms, near-mirror-symmetric realised gaps (**+169.04** and **−171.15** bp against the ladder's −4.02), g50 × ss {0.50, 0.99} × capture {off, on}, 0 gate failures. Capture-OFF, shipped/truth: **the pooled background estimators DO NOT MOVE — intergenic 1.0007 / 1.0000 / 0.9994 and intergenic+intron 1.1817 / 1.1821 / 1.1847 across a 340 bp swing, in BOTH directions.** ⛔ The reason is the pooling, measured not argued: a pooled rate is a ratio of sums, so `ΣE_r/ΣE_g` is **0.9966** (intergenic) and **0.9849** (intergenic+intron) because their exposure is megabase-scale, even at a 169 bp gap (`TRAPS: a-pooled-rate-cannot-see-a-short-object-factor`). ⭐ **Only EXONS are short enough** (`ΣE_r/ΣE_g = 0.7294`, median ℓ 143 bp) and the exon row moves exactly as the factor predicts — the shipped reciprocal bank reads **0.2624 / 0.3439 / 0.8327** of the true total, ordered by RNA length because exons are RNA-dominated. ⭐⭐ **SO THE COMPOSITION HALF BELONGS TO WHATEVER READS EXONS — the landscape (rung ③) and the enrichment ratios — and NOT to the background pools.** ⭐⭐⭐ **AND THE PANEL FOUND SOMETHING BETTER THAN IT WAS BUILT FOR: the shipped estimator's capture-ON error IS A LONG-gDNA-FRAGMENT EFFECT, monotone in gDNA length across three panels.** Intergenic shipped/truth reads **0.9643** at gDNA 79 bp, **0.5627** at 216, **0.5032** at 250, while the pmf-free form is stable at 1.006 / 1.083 / 1.090; the intergenic+intron pool tracks it (0.7567 / 0.2348 / 0.2005). So the "1.8–4.3× capture repair" is really a repair for libraries whose gDNA fragments are LONG, and its size is a property of the library rather than of capture. ⚠ Also visible: the FITTED gDNA pmf reads **105.4 bp** where the library is **78.6** — a 34 % over-estimate, which is the hazard of a pmf in a divisor stated as a number. ⭐⭐⭐ **AND A THIRD FINDING THE EQUAL-LENGTH LADDER CANNOT SEE: THE CONTAINED-OPPORTUNITY MODEL HAS A FRAGMENT-LENGTH-DEPENDENT BIAS AT INTRONS, AND ITS SIGN FLIPS.** `calibration_oracle.py`'s gdna-field-uniformity gate REFUSES to certify densities on both capture-OFF arms: `R intron` reads n/expected **1.0128 (z +13.01)** at gDNA 79 bp and **0.9891 (z −11.04)** at gDNA 250 bp, while the ladder at 216 bp passes 16/16 — so the bias crosses zero near the ladder's own fragment length, which is precisely why an equal-length panel is blind to it. ⛔ It is ±1.2 %, not a catastrophe, but it is a MODELLING error in `(ℓ−w+1)₊` (or in pooling ρ over a reference whose exposure is intergenic while the class under test is short), and it sits INSIDE the shipped estimator's divisor. ⭐ It does NOT touch the arm ⓕ numbers above: their truth is `Σ S_gdna / Σ ℓ`, the START bank over region length, which uses no opportunity model at all — COMPOSITION is certified on all 8 conditions and only the FIELD stamp is withheld. ⚠ Worth a ticket of its own; do not fold it into the abundance work. ⭐⭐⭐ **RUNG ③ IS BUILT AND MEASURED ON ALL 40 CONDITIONS (2026-08-21): the `AbundanceLandscape` — `calibration.abundance_landscape`, layer 5, reusing `landscape.fit_landscape` verbatim on the wall-exact totals with `var = 0` (a direct measurement has no deconvolution ambiguity) — plus a threshold-free MODE CENSUS (every local maximum with its basin; masses carry every verdict; the depleted mode is the basin CONTAINING the pooled intergenic anchor rate; the tolerance is the mode's own fitted width).** Gated by `tests/calibration/test_abundance_landscape.py` (16 cases; 8 perturbations fired; one perturbation — kernel-only `w` — was PROVEN behaviour-preserving on the training population to ~5e-16, because basins cut at density minima that avoid every training kernel's centre, and is documented instead of chased). Integrated at calibration INIT behind `CalibrationConfig.abundance_landscape` (default False ⇒ bit-identical; True refuses without the wall inputs; injectable via the priors bundle for toys). ⭐⭐ **The 40-condition verdict (`abundance_landscape_census.py`): every contaminated ladder row is anchor-CONSISTENT** — gaps 0.003–0.063 nats capture-OFF (`rho_0` 0.0511 vs pooled anchor 0.051 at `g50 ss0.99`; `g98` off reads 0.0997 vs ~0.0998) and 0.25–0.65 under capture; the enriched basin is claimed by EXONS (w̄ₑ 0.35 vs intron 0.002 at `g50` ON) and capture-OFF non-exon w reads exactly 0.0000. ⛔⛔ **THE FIRST REAL FIT CORRECTED THE PLAN'S OWN GATE**: capture-OFF totals are BIMODAL (expression, not enrichment — T's known confound arriving where predicted), so the real invariant is the NON-EXON one, and rung 4 must pair `(rho_0, w)` with the measured enrichment detector (the anchor ratio, 0.98 off / 113–114 on) — off capture the mixture degenerates to the spike REGARDLESS of the total's upper modes, which are RNA. The atlas over all 40 fits is published (Abundance Landscape Atlas). ⛔⛔ **NEXT, IN ORDER.** ⭐ **①** rung 4 — the two-atom mixture through the widened location door (`(m_lo, m_hi, w)`; design in the plan's §3e), consuming `rho_0`+`w` from the landscape AND the enrichment detector, behind `composition_reference` (default bit-identical); then rung 5's pricing with the shuffle control and both zero controls. ✅ **①b THE NPMLE RETIREMENT IS DONE (2026-08-21) and its gate PASSED**: `rename_identity.py --check` reads `✅ BIT-IDENTICAL — quant digest and every array's content unchanged`, which is exactly what had to be true, since nothing the npmle fed was ever read by the solve. Deleted: `npmle.py`, `test_npmle.py` (16 cases), `config.npmle_bandwidth` + its validator, the `--gdna-rate-prior-bandwidth` CLI flag, the `_layers.py` entry and `InjectedCalibrationPriors.enrichment_prior`; `additive=`/`rho_floor=` died unshipped with the module. `CalibrationDiagnostics.from_prior` → **`from_abundance_landscape`**, which is strictly more informative: basins from the mode census rather than a curve's two tallest peaks, a depleted mode picked by the INDEPENDENT anchor, a real `n_train`, and ⭐ **a REAL rug** (the npmle carried no training points, so the report's rug feather was always empty — the landscape now publishes `train_log_rho`/`train_class`). ⛔⛔ **THE DEFAULT FLIP COULD NOT LAND AS §3d WROTE IT, and that is the transferable lesson**: `abundance_landscape = True` against the old REFUSAL broke **65 callers** (24 failed + 41 errors), all one cause — unit and toy fixtures have no wall arrays and never wanted a QC panel. The refusal's own reason was *"rather than fitting on unmasked totals"*, and the alternative was never to fit unmasked but NOT TO FIT, so the missing-walls case now SKIPS with a WARNING and yields `None` (the report omits the panel — its pre-existing contract). ⛔ `background_abundance` KEEPS its refusal because that pair feeds ψ, and a gate now asserts the asymmetry (`TRAPS: measure-a-default-flip-before-you-write-it`). ⭐⭐⭐ **AND THE EVIDENCE THAT SHIPS IT IS NOW MEASURED (2026-08-21, `landscape_head_to_head.py`, all 16): the landscape WINS THE ESTIMAND, and the reason is not the estimator.** Both are the same family of fit; the NPMLE's pair is `mass / eff_gdna` — a total over ONE component's OPPORTUNITY MODEL — so its axis carries that divisor's per-region spread, measured as an offset IQR of **0.12 nats off capture and 1.66 UNDER it** (comparable to the mode separation itself, and removable by no bandwidth). Against certified gDNA truth the depleted mode reads **0.0056 / 0.0264 nats vs the NPMLE's 0.1191 / 0.1266** on the two capture-OFF strata — 4.8–21×. ⚠ **On generic held-out predictive likelihood the two TIE off capture** (−5.36 vs −5.35) and the landscape wins by 0.47 nats under it: read both, because the tie says the NPMLE is not a bad density estimate while the nats column says it is the wrong QUANTITY (`DESIGN.md` §3.1a-iii). ⭐⭐⭐ **THE BANDWIDTH QUESTION IS ANSWERED TOO, AND THE OBVIOUS KNOB IS DISCONNECTED: `_KNN_SCALE` IS INERT AT PANEL SCALE** — `knn_widths` floors every kernel at one grid step and **98.9 % of kernels sit at that floor** at the shipped 0.5 (92 % at scale 2.0), so the smoothing in force is the GRID STEP (`TRAPS: a-floored-knob-is-not-the-bandwidth`). Swept over `_N_GRID` instead, three independent criteria all land on the shipped **260**: the likelihood knee (65→260 buys 1.37 nats, 260→1040 buys **0.006**), the split-half reproducibility MAXIMUM (0.976, falling to 0.889 at 1040), and the anchor-gap collapse (0.126→**0.003** nats). ⛔⛔ **AND THAT SWEEP FOUND A DEFECT IN RUNG 4's PLANNED INPUT: `span_R` IS GRID-FRAGILE** — 58 → 95.6 → **1.9** on `g50 ss0.99` capture-OFF, because `split_basins` picks the enriched mode by BASIN MASS and over-resolution fragments the bulk into competitors. `rho_0` (8–25 % over a 16× range) and the anchor verdict (**12/12 at every grid**) are robust; `span_R` and the mode COUNT are not (`TRAPS: a-mode-count-is-not-a-well-posed-quantity`). **So rung 4 consumes `rho_0` + `w` + the enrichment detector and NOT `R` as published.** ⭐⭐ **THE TRANSFER-VARIANCE AUDIT IS DONE (`transfer_variance_audit.py`) AND ITS ANSWER IS NO, FAILING IN BOTH DIRECTIONS AT ONCE.** `DensityNPMLE.project` is confirmed at ZERO `src/` callers, from the AST (the formula survives in one docstring as provenance, which is correct). ⛔ Its replacement `transfer_logvar` is counting-plus-composition, so **every term shrinks with depth where the retired between-mode term did not** — measured, **25.6 % of real hops carry the whole damping term below 0.1 nats² off capture**, i.e. a deeply-counted transport arrives essentially undamped; `currency.premise_logvar` fills that hole, so **the gap is `RelayPolicy`'s alone rather than the tool's**. As a replacement, the landscape's per-slot grid posterior is **0.92× the counting variance off capture** — TIGHTER, the wrong direction, because a fitted population prior shrinks a slot toward a mode — while its population spread **over-states a fitted premise by 7.4–8.0×**, since it contains the real enrichment structure the message is trying to transport (signal, not uncertainty). ⭐ What survives: under capture the posterior LOOSENS (1.03×), and that increment IS mode-membership ambiguity — the retired term reappearing legitimately, on the right substrate and in the right direction, and the honest seed of the per-hop-type premise `premise_logvar` already names as the obvious refinement. ⛔ Composition-vacuity is unmoved: a TOTAL licenses a LEVEL hop's variance and the population premise, never a COMPOSITION hop's. **②** enrichment ratios (`currency.enrichment_ratio` at exon hops — the ~3× truncation; `region_total_density` → the relay reframe): same reason, same population; ⚠ blocked by the policy freeze. **③** `density_model.region_gdna_density`, now DEMOTED — its own branch is a pooled long-region rate the panel just showed is insensitive to the gap, so its only remaining case is capture-ON robustness (0.7567 → 1.0445 at gDNA 79 bp; 0.2005 → 1.0544 at 250), which is the same win the landed swap already earns. ⛔ **④** the fl-gap panel is DONE — do not rebuild it. What it has NOT priced is the DELIVERABLE end to end: `calibration_vs_oracle.py --background-abundance` on the two arms is the outstanding run, and it is the one that would justify flipping the default. ⚠ `additive=` and `rho_floor=` in `npmle` are also passed by nobody, and `enrichment_prior` reaches only QC + toy injection — a SEPARATE converge-and-delete candidate, owner's call | ⛔⛔ **Every cheap alternative is now measured-refused on the current tree**: the library-wide truth mean (2.65× at `g05` stranded × ON; a cancelling pair inside a 1.001× total at `g98 ss0.50 off`), and BOTH single-mode per-object locations — `pooled` AND the flank-cancellation `local` — at 5–8× on stranded × ON with identical `B exon\|exon` damage (a near-vertex location is a strong claim in nats whatever the strength budget). ⚠ The equal-length panels are a NULL for the composition half of the abundance fix BY DESIGN; the truncation and wall halves are visible, and the fl-gap SIDE panel (never a ladder rung) is what fully exerts it. ⛔ The small-exon length-composition channel is DEFERRED (owner, 2026-08-21), with the 0.8.0 retirement; `region_span_count` stays dark behind its ledger invariants |
