...
| | | |
|---|---|---|
| ⭐⭐⭐ **THE THIRD POLICY EXISTS, AND IT IS THE FIRST ARM TO BEAT BOTH SHIPPED POLICIES ON EVERY IN-SCOPE CONTAMINATED STRATUM** | ⭐ **NEW 2026-08-20.** `messages/currency.py`, beside `SilentPolicy` and `RelayPolicy` on the same gated backbone; the A/B is one config value (`CalibrationConfig.message_policy`) and `RelayPolicy` is untouched. Measured on the Stage-3 test chromosome, gDNA **absolute error in FRAGMENTS**, Silent / Relay / **this**, ⛔ never pooled: unstranded × capture-OFF 673 / 2,236 / **662**; stranded × capture-OFF 619 / 1,940 / **617**; stranded × capture-ON 5,632 / 19,086 / **4,570**; ⛔ DEFERRED unstranded × capture-ON 65,707 / 36,158 / 43,171 — the honest cost of a weak imputation on the one stratum with no strand channel. Zero controls 95,757 / **2** / 31 and 31,288 / **99** / 4,935: the relay is still ahead there, and `§0`'s reference row says why that is a REFERENCE result rather than a message one. | ⭐⭐ **The design is FOUR rulings, all owner-set and all in `DESIGN.md` §0c.0b–d**: the two strategies are ONE continuum with a FITTED knob (`EQUATIONS.md` §3.5f); the RECIPIENT decides, from a static population table keyed per (boundary, side); an imputation is a WEAK predictor and pays a fitted PREMISE on every hop (§3.5h); and pass-0's job is to solve ENOUGH exons to train a prior, not all of them |
