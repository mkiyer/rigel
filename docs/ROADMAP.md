...
re-derives it.** "The relay costs the in-scope contaminated strata, and the currency policy wins every
zero control — re-derive with `relay_pool_ab.py`" is durable; twelve six-digit fragment counts are not.
...
| ⛔⛔ **THE THREE POLICIES — THE STANDING BENCHMARK** (`relay_pool_ab.py`, gDNA absolute error in FRAGMENTS, all 16 conditions; render it with `benchmark_report.py`) | `SilentPolicy` wins all three **IN-SCOPE** contaminated strata; `CurrencyPolicy` wins **all four zero controls**, by a wide margin at three of them; `RelayPolicy` wins only the DEFERRED stratum, where it beats the others by a large factor. ⭐⭐ **The verdict, which is what to carry**: propagation is net-harmful wherever the local solve HAS evidence, for BOTH message policies, and its value is concentrated where that solve is BLIND. ⛔ Ranks 1 and 1a design the production policy from the CONTRAST between the three, not by picking this table's winner |
| ⚠ **the weighted-premise fix, and the SUBSTRATE warning that goes with any policy claim** | It moved most rows of the table above, by up to a small integer factor on the worst of them, and changed the RANK on **no** in-scope stratum. ⚠ On the **TEST CHROMOSOME** `CurrencyPolicy` is best on all three in-scope strata — the opposite ordering — so every claim about a policy must name its substrate (`TRAPS: a-toy-and-a-panel-can-disagree-in-rank`, `TRAPS: panel-before-src`) |
...
| **1a** | ⭐⭐ **CONTRAST THE THREE POLICIES — the differences are themselves the design instrument** (owner). `SilentPolicy`, `RelayPolicy` and `CurrencyPolicy` fail DIFFERENTLY; the production policy is designed from that contrast, not by picking a winner | Message propagation is net-harmful on every in-scope contaminated stratum for BOTH message policies, and its value concentrates where the local solve is BLIND (the deferred stratum, both capture-ON zero controls). ⭐ So the cheap decisive question is whether a policy that speaks **only** into destinations with no own evidence beats one that speaks everywhere — a measurement, not a switch. ⛔ Score it split by whether the destination had its own composition evidence (`TRAPS: an-imputation-must-cost-something-every-hop`), on `relay_pool_ab.py`, and name the substrate on every claim (`TRAPS: a-toy-and-a-panel-can-disagree-in-rank`) |
...
| **1d** | ⭐⭐ **THE FAN-OUT'S PREMISE VARIANCE — the DEFLATION HALF IS LANDED (2026-08-23); what remains is the INFORMATION CEILING.** `FanOutPolicy` now deflates its stage-4 exon λ claim by the shipped DerSimonian–Laird mismatch variance (`messages.variance.mismatch_deflate`, the relay's own law on the same axis) against the destination's own λ solve. ⭐ Derived, no constant, one guarded block, and **no shipped default moved** (`message_policy` stays `"relay"`). Measured on the ladder at claimed slots: the worst stranded × capture-ON rung goes from +125 % over silence to **+6.5 %**, one contaminated rung becomes a **WIN**, and **every unstranded condition, both zero controls and the DEFERRED stratum are byte-identical** — by the algebra (`τ_λ ≡ 0` at κ = ½ ⇒ `v_own = ∞` ⇒ `b̂² = 0`), not by a gate. Gates: six cases in `tests/calibration/test_fanout_policy.py`, two verified failing first; the perturbation sweep's two misses are provably semantically inert. | ⛔⛔ **WHAT IS LEFT, and it is structural**: the high-gDNA rung keeps most of its residual, because the DL test is RELATIVE and cannot fire where the destination's own λ precision has itself collapsed (`I_strand ∝ [f_g(1−f_g)]²` → 0 as `f_g → 1`). That needs an INFORMATION CEILING on the delivered precision, and the blunt form (`tau_e ≤ tau_b`) is measured-**DISQUALIFIED**: it regresses the deferred stratum ~3×, because there the collapsing Jacobian is doing legitimate work. A ceiling GATED to destinations that have own evidence is a cousin of the deflation, not an independent lever — price one honestly before building it. ⭐ Re-derive any of this with `pass0_claimed_ab.py [--dissect]` |
| **1d″** | ⭐⭐⭐ **THE ORIGINAL DIAGNOSIS, kept because it is the evidence rank 1d and 1d′ both rest on.** `FanOutPolicy` wins decisively where exons are blind and regresses at stranded × capture-ON exons; `pass0_claimed_ab.py --dissect` establishes that the whole regression is stage 4's λ transfer, that the implementation carries NO bug, and that the delivered claim's precision is over-claimed by an order of magnitude and more against certified truth while the exon's OWN side is honest. ⭐⭐ **The cause is the OWNER'S PROBE-PLACEMENT MODEL (2026-08-23)**: capture behaviour at an exon flank depends on where the probe sits — internal to the exon (composition intact, crossings DEPLETED), absent (the exon is capture-OFF), or SPANNING A SPLICE JUNCTION (spliced fragments fully overlap the probe and enrich, unspliced boundary-crossings barely do, biasing the transferred composition hard toward RNA). ⭐⭐ **Measured on the panel, the DOMINANT term is none of the three individually but their common cause: EVERY probe base lands in an exon and introns carry EXACTLY ZERO, so an intron\|exon boundary is a PROBE CLIFF by construction** — the flank's crossing straddles unprobed sequence while the exon interior it speaks to is a quarter probe-covered. The panel does express splice-junction-spanning probes (about a quarter of them, tiled in transcript coordinates), but matched on local probe fraction the capture-attributable deficit is LARGER where they are absent, so a fix aimed only at them addresses the smaller half. ⛔ Probe coordinates are unavailable for real panels, so the composition bias CANNOT be removed — deriving the enrichment landscape at base resolution without the panel is the elegant answer and is OUT OF SCOPE for 0.8.0 | ⭐⭐⭐ **So the ruling is DAMPEN, and the instrument already exists and is DERIVED rather than tuned**: `messages.variance.mismatch_deflate`, the DerSimonian–Laird between-source mismatch variance `b̂² = max(0, G² − v_msg − v_own)`, whose closed form states the safety property exactly — *a message may out-weigh the destination's own belief only when it agrees with it to within `√2·σ_own`*. ⛔ **`fanout.py` imports NOTHING from `messages/variance.py`** — it is the only speaking policy with no premise-variance law at all, which is the defect stated structurally. ⭐ Probe placement is unobservable but its FOOTPRINT is not: the observed λ gap between the claim and the destination's own solve is what the deflation reads. ⛔⛔ **THE GATE, and it is why this is ranked rather than merely proposed: the three DL regimes map onto the strata for free** — where the destination has NO own composition evidence (`τ_λ ≡ 0` at κ = ½) `b̂² = 0` and the message passes UNTOUCHED, so **every unstranded condition is byte-identical, both zero controls included, and so is the DEFERRED stratum — which is the fan-out's design target and its largest win.** That is a derivation, not an empirical hope. Measured with `--dissect`'s arms on the stranded × capture-ON rungs: the worst cell's exon regression nearly vanishes, one contaminated rung turns into a WIN, and the stranded capture-OFF residual shrinks too. ⛔ **Deflate the STAGE-4 exon claim ONLY** — applying it to the stage-3 boundary claim as well costs boundary wins, and stage 3's premise is certified by the stage-0 matrix while stage 4's is the one that fails. ⛔⛔ **IT IS NOT A COMPLETE FIX AND THE LIMIT IS STRUCTURAL**: the DL test is RELATIVE, so it cannot fire where the destination's own λ precision has itself collapsed — `I_strand ∝ [f_g(1−f_g)]²` vanishes as `f_g → 1`, which is exactly the high-gDNA rung, and that rung keeps most of its residual. ⭐ **The obvious constant-free ceiling — a transferred belief may not out-certain its source, `tau_e := min(tau_e, tau_b)` — IS the larger lever on the calibration reading and reaches the high-gDNA rung the deflation cannot, but it is MEASURED-DISQUALIFIED as stated: it regresses the DEFERRED stratum by ~3×, wrecking the fan-out's largest win, because there the collapsing Jacobian is doing legitimate work.** A ceiling gated to destinations that HAVE own evidence is a cousin of the deflation, not an independent lever; pricing one honestly is the still-open half. ⛔ `median |z|` is an INTERMEDIATE — rank on the claimed-slot error (`TRAPS: the-intermediate-is-not-the-deliverable`) |
