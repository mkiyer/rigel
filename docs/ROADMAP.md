...
|---|---|---|
| **1** | ⭐⭐⭐ **THE DISSECTION LOOP, ON THE WORST IN-SCOPE SCENARIO — the owner's protocol, 2026-08-20, and it is the method rather than one task** | ⛔ **In order, and the dissection is the step that must not be skipped**: ① run the FULL panel, all three arms, every scenario reported; ② pick the worst scenario **within the three IN-SCOPE strata** (never the deferred one, which is otherwise worst every time); ③ find the REGIONS and BOUNDARIES carrying the highest error MASS and, for a SAMPLE of them, trace the error to its root cause — from INITIALIZATION, through every belief change, every message and every refit, each step compared against the certified `slot_truth`; ④ design a fix, gated first; ⑤ **add the offending transcripts to the TEST CHROMOSOME**; ⑥ re-run the panel and see how the fix behaves everywhere, not only where it was designed; ⑦ repeat on the next highest source. ⭐⭐ **Step ⑤ is what makes the loop compound**: the test chromosome becomes a repository of the transcript combinations the tool actually fails on, and eventually stands on its own as the benchmark for comparing calibration algorithms | ⛔ **THE FIRST THING TO FINISH is the open lead**: the new policy's premise variance — the term that keeps an imputation weak — fits by method of moments, and the UNWEIGHTED fit is dominated by the sparsest hops. On the ladder the median slot holds **13** fragments and a fifth hold NONE, so `mean(v_r) = 2.07` swamps `Var(log r) = 0.76` and the premise floors at **0.0**; on the test chromosome (median 354) it fits 0.992. ⭐ A PRECISION-WEIGHTED fit returns 0.294 / 0.324 on the two substrates — one number, as a library-level property should be — and is implemented and gated; **the panel re-run with it was in flight when the session ended.** ⚠ It may or may not close the gap; if it does not, the dissection loop is the answer |
