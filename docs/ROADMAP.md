...
|---|---|---|
| **1** | ⭐⭐⭐ **CLOSE THE ZERO-CONTROL GAP THE PER-STRAND LICENCE OPENED** | The relay's composition licence was repaired 2026-08-18, twice over, by the dissection loop: ① the licence only gated the gDNA arm — the RNA arms were reframed by the FULL total-density ratio at every hop, so a TSS− boundary crushed a continuing + population 7× and ψ booked the deficit as gDNA (true 0.0023 → solved 0.9009); ② the v1 repair (every arm unscaled at a denied hop) flooded same-strand tandem genes (+115 k fragments, flags read TES+\|TSS+). The landed rule is PER-STRAND (`rna_level_scale`): both populations intact ⇒ reframe by `r`; this strand intact, other changed ⇒ `r = 1` (a DENSITY transfer); this strand changed ⇒ NO CLAIM. ⭐ All four strata improved vs the pre-licence relay (0.92/0.93/0.77/0.87) and all three convicted slots land on truth. ⛔ **THE OPEN BAR: the zero controls went 154 k → 431 k**, worst at `g00 nrna_mid capture_on` (62 k → 228 k). ⚠ Part of what the rule removed was the SELF-CONFIRMING ECHO, coincidentally always "right" at a zero-gDNA library — but whether an honest channel (the graft where a terminus and an sj share a boundary, ~1 % of boundaries) was also silenced is UNMEASURED. Dissect that condition v2-vs-pre with the same loop; the session handoff in the dev sandbox carries the step-by-step |
| **2** | ✅→⭐ **THE RELAY IS RE-PRICED (twice, 2026-08-18) AND ON** — what remains is closing its ~1.5–1.65× cost on the in-scope contaminated strata, bug by bug | Re-priced on the 32-condition nascent ladder against certified truth (`relay_pool_ab.py`): a large win on unstranded × capture-ON (0.31× vs muted) and every zero control, a 1.5–1.65× cost on the three in-scope contaminated strata (down from ~2.0× pre-licence). The zero-gDNA golden catastrophe (0.029 → 89.93) is GONE — those goldens now read ~0.007–0.04 with the relay on. ⚠ The 2026-08-07 mute table is history: measured with the licence bug live on a retired panel | ⭐ Candidates in the queue, measured once each at the crime scene: `transfer_var` off read 0.0010 (better than either licence fix alone — understand before shipping); `strand_population` measured INERT on this panel (annotation covers flips with TSS/TES bits) — keep or cut is an owner call |
