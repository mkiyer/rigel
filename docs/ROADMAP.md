...
|---|---|---|
| **1** | ⭐⭐⭐ **REBUILD MESSAGE PROPAGATION AS A THIRD POLICY — owner-ruled 2026-08-18, and it REPLACES the per-bug loop that used to sit at this rank** | ⛔ The owner's ruling: *"the whole method from the top down is not simple, and it is not elegant … it can be simple and elegant"*. `RelayPolicy` carries **19 switches**, and a census of the panel says the problem needs **six hop types**. ⭐⭐ **THE PRINCIPLE**: a hop carries a **LEVEL** (gDNA fragments per placement) or a **COMPOSITION** (the gDNA share), and the two have exactly COMPLEMENTARY invariances — a LEVEL survives a POPULATION change (gDNA knows nothing about transcripts) and dies at an ENRICHMENT change; a COMPOSITION is the reverse (a probe enriches both components alike). So at every hop at least one is intact, and the licence question IS the currency question, asked once per hop type. ⛔ **The shipped relay has this backwards**: it transports a COMPOSITION by default and bolts population licences on top — but the population is exactly what changes at the `exon\|exon` boundaries carrying the largest destination mass. Every licence bug of 2026-08-18 was repairing a currency CHOICE instead of changing it. | ⭐⭐ **THE RULING IS `DESIGN.md` §0c.0** (the two currencies, the direction rule, and the three settled points); the working plan and its rungs live in the dev sandbox. Stages 0 (VOCABULARY), 1 (UNBLOCK — the chimera drop, FIELD certification 24/32 → 32/32) and **2 (THE MAP — `hop_currency.py`, all 32, §0's row) are DONE**. ⭐ **NEXT, IN THE OWNER'S ORDER (2026-08-19)**: ✅ ② the SIMULATOR on the rigel index is **DONE** (§0's row); then ① the map PER ARM — `hop_currency.py` extended to `{gDNA, RNA+, RNA−}` (ruled ESSENTIAL; the first pass pooled the strands), ③ the toy harness on the new simulator, ④ all 32 re-simulated and re-certified, ⑤ the policy **TOY-FIRST, one transcript at a time**, its arithmetic `EQUATIONS.md` §3.5e — three densities, rescale-by-totals at a compatible hop with SPLICE OUT / SPLICE IN, level only across a terminus (`sj+term` is a terminus), the static hop table keyed on `object class × {sj, term, sj+term}` (`TRAPS: an-object-class-does-not-see-a-terminus`). ⛔ Then a third policy beside `SilentPolicy` and `RelayPolicy` on the same gated backbone — `RelayPolicy` untouched, so nothing that works can break and the A/B is a config value — verified rung by rung on toys (ending at the owner's TA–TF locus), panelled in RAW COUNTS, and only then is `RelayPolicy` retired |
