...
**Tools:** rigel (C++ EM solver), salmon 1.10+, kallisto 0.51+

---

## 1 Executive Summary

After porting the EM solver from Python to C++, rigel retains its
...
- **Pristine:** rigel MAE 27.1 vs kallisto 29.8 vs salmon 39.7
  (rigel 1.1–1.5× better)
- **gDNA/nRNA contaminated:** rigel MAE 0.9–1.8 vs salmon 31–50 vs
  kallisto 35–92 (rigel **18–50× better**)
- **Gene-level:** rigel MAE 0.4–5.4 vs salmon 277–378 vs kallisto
  280–736 (rigel **50–145× better** under contamination)

Speed remains the weak point: rigel 17–71s vs salmon 1.0–1.4s vs
kallisto 0.9–1.3s.
...
Under pristine conditions (no gDNA, no nRNA), rigel's transcript-level
...
- **Hulkrna gene-level pristine MAE is 0.42** (vs 0.17 for kallisto),
  ...
  rigel's isoform deconvolution adds some noise in the absence of
  confounders.
...
- The low correlation is a **small-signal regime artifact**, not a
  fundamental accuracy issue — rigel's MAE is still 31× better than
  salmon.

**Study opportunities:**
1. Log-scale or rank-based metrics may better capture rigel's advantage
...
rigel's (0.42). This suggests rigel's mRNA/nRNA/gDNA decomposition
...
rigel has **0% dropout** (every truth transcript gets non-zero counts),
...
2. **rigel dominates accuracy** especially under contamination (18–145×
