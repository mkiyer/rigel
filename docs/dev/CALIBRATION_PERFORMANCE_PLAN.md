...
| W13 | **The port's prerequisites** (§8) — DONE 2026-09-13 on `23a431a9`, RE-TAKEN 2026-09-14 on the L5 tree after the lanes worklist moved numbers: `sweeps_MO_3021_step6` (four calls, each replays BIT-IDENTICAL: 30.0 / 12.9 / 13.0 / 13.1 s), `port_identity_{gdna_g05_ss_0.50_nrna_mid_capture_off, gdna_g05_ss_0.99_nrna_mid_capture_on, LBX0190}.json` re-frozen and `--check`ed (the `23a431a9` set in `arms/port_identity_23a431a9/`), and the deep-library baseline as two back-to-back pairs, `perf/baseline_2026-09-14/pair{1,2}_{a,b}.json` (wall 498–502 s, peak 10.8–11.2 GB, `calibrate` 404–408 s, every stage 0.96–1.02 within a pair) | re-capture the deep library's sweeps on the current tree (`sweep_replay.py capture`), since `sweeps_MO_3021_step4` predates the lattice; fresh identity references for the port thread | the captures replay bit-identical on the tree that made them; `rename_identity.py --check` |
...
| L6 | **The landscape's location floor** — DONE 2026-09-14 (`DESIGN.md` §7.1 rule 4; `ISSUES: gdna-landscape-trains-on-false-positives` (d) CLOSED): a slot trains only where it holds a composition AND its solve locates it, `Var(log f_g) ≤ 1 nat²`, the E-step's one-fragment floor through `Var(log c) = 1/c`; the ladder's four zero controls 500 → 282 / 211 → 194 / 550 → 265 / 231 → 172, every stratum unchanged or better on both panels | the owner's direction: which nodes may teach the circular half | the metric per stratum, both zero controls, the training census, the walk; one gate, three perturbations |
| L5 | **The witnessed atom** — DONE 2026-09-14 (`DESIGN.md` §6b.15, `EQUATIONS.md` §9f, the issue CLOSED with its numbers; the unwitnessed cost filed as `ISSUES: the-atom-at-an-unwitnessed-both-strand-slot`) | the AMBIG tilt's hypothesis space {pure +, pure −, mixed} at equal reference weight, two atoms in ψ's cube beside the continuum (`−log π` on its weights), a held level on a strand ruling the other strand's atom out | the metric per stratum (stranded ON −9.3 %, every stratum better), both zero controls (stranded +53 / +7), the census bands, both stresses, the encompassing gate un-xfailed; gates in `test_vertex_reference`, four perturbations |
