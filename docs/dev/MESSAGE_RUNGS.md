...
| 5 | both-stranded block | strand-change faces, both-stranded loci, the AMBIG tilt channel | ⚠ PARTIAL — the strand-change faces carry the level lane (§6b.12); ✅ 2026-09-08 THE RNA LEVEL LANES (`DESIGN.md` §6b.13): per-strand faces from the flag bits, the sources (own strand profiles, the certified flux at an exon's junctions), delivery at AMBIG nodes on the cube; the tilt needs no lane. Ladder: every non-zero row of both halves won. Owed: phase 2 (RNA levels at SINGLE-STRAND recipients — the ceiling), then the gDNA lane's reach (`ISSUES: levels-always-travel-for-the-gdna-lane`, refused by the bar), `ISSUES: flux-floor-dispersion`, `ISSUES: ambig-node-as-a-gdna-source` |
...
| sj-plus-terminus | a boundary with sj AND terminus (386 on the ladder) | 386 | the terminus decides the side; rule 8 inside, item 5 outside, the junction's flux placed at its exon's flank | both | ✅ 2026-09-08 (`DESIGN.md` §6b.14): neutral in scope (the case is 0.6 % of a row at the counting floor), −14 % locally on the unstranded zero row |
| termini-both-ways | boundary with termini BOTH ways | 48 | **level** only | level | ⏳ with the level message |
| ambig-tilt | AMBIG boundary / the tilt channel | 4,671 | the RNA level lanes (both strands' bounds constrain the tilt through the shares) | the cube row | ✅ 2026-09-08 (`DESIGN.md` §6b.13): the tilt has NO lane of its own — a tilt profile from the same witnesses would count them twice; the AMBIG node's λ and θ are bracketed by the three levels (THE BRACKET THEOREM, gated) |
| strand-change-face | strand-change face | 249 + 889 | **level** only (composition refused: membership changes) | level | ✅ 2026-09-05 THE LEVEL LANE (lower-only, `DESIGN.md` §6b.12) |
...
|---|---|---|---|---|---|---|
| 1 | **THE LEVEL RULE** — the inside of every terminus (✅ landed 2026-09-04 as rule 8, both terminus kinds), then strand-change faces, termini both ways, the EMPTY chain pieces between termini | STOP by omission (no rule) at the remaining faces | 1,138 + 48 + 5,588 boundaries (3,792 reached only through another terminus) | terminus boundaries 12–15 % OFF / 28 % ON; walled exons 8–10 % / 18 % | the landed rule's laws (a level from the measurement only, shape preserved, per-pair widths, the total's upper bound) extended to the faces where composition cannot cross; the empty piece needs the level LANE forwarded | ✅ 2026-09-05 — THE LEVEL LANE serves every face without a composition rule, empties forward it; STOP by omission gated impossible |
| 2 | **sj + terminus** faces | STOP by omission (`outside_flank`/`junction_flanks` return none) | 386 | inside the terminus / alt-ss shares | the terminus decides the side; the flux placed at the junction's exon flank | ✅ 2026-09-08 (`DESIGN.md` §6b.14) |
...
| 4 | **AMBIG tilt** and the both-stranded locus | a ruling, not a hole (rung 5) | 4,671 + 5,241 | 6–13 % of mass | THE RNA LEVEL LANES | ✅ 2026-09-08 phase 1 landed (`DESIGN.md` §6b.13, the record `AMBIG_DESIGN.md` §4b); phase 2 (single-strand recipients) is the next step and the gate for the gDNA lane's reach |
| 5 | **substrate**: `nest` (`instart`'s case is the sj+terminus block's `S`, 2026-09-08) | nothing to falsify 1–2 on | — | — | one YAML structure per step | ⏳ |
...
| 9 | an alternative FIRST exon inside an intron (`exon\|intron[term]`) | the sj+terminus block's `S` (2026-09-08; `instart` retired as a name) | the intron is always the outside flank; the inside exon's other face | ☐ |
