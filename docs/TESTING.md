...
method development benchmark."* It was cleared to zero on 2026-08-27 and has grown in three blocks since
...
exon\|exon TERMINUS boundary; 10 genes). Queued, each added on the day it becomes the target: `altss`,
`nest`, `instart`. U by the mature ladder, T by the ANTI-ladder (100/30/10/3/1); a − gene's geometry is
mirrored within the span so each type keeps its meaning.
...
the simulator disagree with the annotation. Generating the chromosome from a fixed seed (the YAML's `genome_length`, 2.9 Mb since the isoform
...
Editing that block changes nothing. ⭐ Depth is `n_total_fragments: 480000` per condition since the
isoform block (raised from 200 k to hold per-transcript depth as the chromosome tripled its molar mass).
