...
capture panels and the FASTA are all RENDERED from it by `build_test_reference.py`, and
`tests/test_test_reference_renders.py` refuses a render that drifted. ⭐ **Every gene carries an explicit
strand and the chromosome keeps equal + / − representation** (owner, 2026-09-02: a sign error is invisible
on a one-strand substrate; the builder refuses an imbalance) — this is not the both-stranded locus, which
is a later step. The YAML's header is the design record of every block; in brief:
...
⭐⭐ **THE MONO BLOCK** (2026-09-02) — 20 single-exon transcripts (`mono` / `capmono` expressed, `monosilent` /
`capmonosilent` at `mrna = 0`, × the five blocks): no sj, no spliced RNA, the ONLY message source is the
gene edge (rung 3 of the transfer policy).

⭐⭐ **THE ISOFORM BLOCK** (2026-09-02) — multi-exonic, MULTI-isoform, single-stranded transcripts, grown ONE
structure at a time (owner ruling the same day): host `U` = the twin shape plus ONE second isoform `T`,
probed and unprobed, × the five blocks. Present: `altstart` (T starts inside U's middle exon → one
...
twin host whose exon PIECES have NO licensed face, the four classes the ladder's walled-exon census
found that a scan can reach (11,350 ladder slots, 2.0 M fragments: 35 % between two termini in chains
of ~20-bp pieces, 24 % junction\|terminus, 17 % junction\|junction, 17 % against a refused intron face).
`chain` (three alternative TSSs 40/100/240 bp into exon 1 → two walled pieces in a row, 60 and 140 bp),
`tssalt` (a TSS 300 bp into exon 2 beside an alternative donor 600 bp in → one piece,
terminus\|junction), `tandem` (an alternative acceptor and donor on exon 2 → one piece between two
junctions, 600 bp), `altlast` (two isoforms ending in an intronic last exon by different acceptors and
TESs → two pieces in a row, the last against a REFUSED intron face); × ab / ba / eq × unprobed / probed ×
abundance blocks 2 and 4 = 48 genes. The refused\|refused class (an exon inside an intron) is the level message's
...
| piece | |
|---|---|
| `scripts/sim/test_reference/test_chr.yaml` | ⭐⭐⭐ **THE ONE HAND-EDITED FILE.** Genes with explicit strands, transcripts with exons and abundances, `probed` per gene, and the `shadow_genes` on the BLANK chromosome `test_blank` (owner design, 2026-08-29: unannotated transcription the SIMULATOR draws from and the INDEX never sees — nascent always 0, a shadow IS the unannotated RNA; what it controls is anything that trusts the annotation's notion of "pure gDNA", the gDNA strand-overdispersion fit above all). This is where a structure is added |
| `test_chr.gtf` · `test_shadow.gtf` · `test_abundances.tsv` · `test_probes.bed` · `test_probes_sparse.bed` · `test_probes_junction.bed` | ⛔ **RENDERED** from the YAML by `build_test_reference.py` and versioned beside it so a reader sees the annotation without running anything; `build_test_reference.py --check` (and the suite gate) refuse a render that drifted. The benign panel tiles each probed gene's exon UNION 8 × 125 bp seamless, never spanning an sj; the sparse panel centres one probe per annotated exon; the junction panel places one two-block probe per sj |
...
#    simulate, both caches, the g00 pre-warm + _main copy, certify
CFG=scripts/sim/configs/test_reference.yaml; OUT=$T/scenarios
python scripts/sim/panel.py simulate --config $CFG --index $T/idx
python scripts/sim/panel.py cache    --config $CFG --index $T/idx
for C in $OUT/gdna_g00_*; do c=$(basename $C)
  python scripts/design/pass0_vs_oracle.py --suite $OUT --index $T/idx --oracle-cache $OUT/oracle_cache --_prewarm $c
  rm -rf $OUT/oracle_cache/$c/_main && cp -r $OUT/scan_cache/$c $OUT/oracle_cache/$c/_main
done
python scripts/design/calibration_oracle.py --suite $OUT --index $T/idx
...
describe the annotation they were built from; a benchmark run against stale caches answers a different
question and says nothing about it (`TRAPS: a-green-suite-hid-five-dead-instruments` is the same failure
in another dress). ⚠ Each superseded derived set was moved to
`~/Downloads/rigel_runs/test_reference_STALE_<n>tx_<date>/` on the day rather than left in place for exactly
this reason (42 tx on 2026-08-27, 45 tx on 2026-09-02).
