...
| # | Plan §       | Plan called for                                                                | Shipped (HEAD)                                                                                         | Decision / status     |
|---|--------------|--------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------|-----------------------|
| 1 | M7           | `compute_pool_fl_models` returning `PoolFLModels`                              | `build_fl_models` returning `FLModels`                                                                | **Documented (M9.1.b)** — plan §M7 amended in commit `<M9.1.b>`. |
| 2 | M8 (CLI)     | `--cal-quality-good`, `--cal-quality-weak` driving `pool_quality_thresholds`   | Shipped in M9.1.a (commit `<M9.1.a>`)                                                                  | **Resolved** — flags + `CalibrationConfig.{pool_quality_good,pool_quality_weak}` + 4 tests. |
| 3 | M8 (Config)  | `CalibrationConfig.pool_quality_thresholds=(5_000, 200)`                       | `CalibrationConfig.pool_quality_good=5_000, pool_quality_weak=200`                                     | **Resolved (M9.1.a)** — split into two scalar fields (argparse can't bind a tuple). |
| 4 | M8 (summary) | `summary.json` keys: `kappa_diagnostics`, `boundary_flux_gdna_summary`, `global_densities` | Keys: `global_densities`, `fl_models`, `diagnostics`, `n_multi_loci`, `c_base`, `mean_pi_gdna`         | **Documented (M9.1.b)** — `boundary_flux_gdna_summary` folded into `diagnostics`; `kappa_diagnostics` folded into `global_densities`.  Schema is documented in `docs/parameters.md`. |
| 5 | M8 (deprec.) | v1 CLI flags become deprecation-warning shims for one release cycle             | v1 flags removed outright in M8c                                                                      | **Documented (M9.1.b)** — rigel pre-1.0; deprecation cycle skipped.  Noted in `CHANGELOG.md`. |
| 6 | M8 (`__init__.py`) | exports include `PoolFLModels`, `compute_pool_fl_models`                  | exports include `FLModels`, `build_fl_models`, plus `extract_global_counts`, `extract_rna_counts`, `extract_gdna_counts` | **Documented (M9.1.b)** — superset of plan; no breaking change. |
