...
| **P4b** | ⚠ **THE REMAINING BP VIOLATION — `_RHO_ITERS = 2`** | ▶ open. The 2nd iteration's reframe faces are built from the destination's **fused posterior**, i.e. from the other message. Measured median \|Δlog ρ_face\| between iterations 0.0116 (stranded capOFF) → 0.1242 (unstranded capON), >1 % on 52.7–79.0 % of nodes. Deleting `_far` removed the largest violation, not the last one |
