...
| **P-merge** | merge `_relay` / `_transport` (the near-twins) | ⛔ **MEASURED AND REJECTED** 2026-07-26. Structurally possible (`_peel_share` proves the `k` = scalar-or-slice pattern), but `_relay` is a SEQUENTIAL Gauss-Seidel scan whose per-node arithmetic is plain Python floats; routing it through numpy costs **15.7×** per op (0.083 → 1.307 µs) = **+5.5 s per calibration** at 74,494 nodes, to save ~60 lines. The twin relationship and the three deliberate edge-case differences are now documented at BOTH sites |
