...
| ⭐⭐⭐ **C12** | **THE TWO FACES OF AN `intron\|exon` EDGE — component-set matching, derived and VERIFIED, not built.** One EDGE, one gDNA density, two composition statements: the INTRON face uses the unspliced population only, the EXON face adds the junction. `EQUATIONS.md` §3.6 | ⭐ identity verified to **+0.000/−0.003** in `phi_g` at m=100, and **+0.027/−0.020** on the nascent control (Poisson on 9–15 edge counts). ⭐⭐ Worth, at m=1 on the exon the shipped solve gets most wrong: `f_g` **0.625 → ~0.499** against a truth of 0.458, by referencing the intron's 349 counts instead of the edge's 8 | ⭐⭐⭐ **derived + verified against oracle truth on both faces, with and without nascent** | medium. ⛔ **A RE-SOLVE ceiling first** — the substitution ceiling understates a message source (`TRAPS.md` B17) |
| **C13** | **`E_J` vs the exon's `E_r` disagree by 5–10 %** — the residual of C12's exon face. `(rho_nas+rho_mat)/rho_R(exon)` = 1.103 (no nascent), 1.049 (with) | ⭐ measured; it is the whole residual of face (II) | ⚠ not diagnosed — either `E_J` slightly too small or the exon's `E_r` slightly too large | small |
| **C14** | ⛔ **`bp_solver`'s P1d asserts the graft is a LOWER bound and it is an UPPER one.** `rho_R(exon) ≥ rho_nas(B)+rho_mat(B)` is used as an equality; measured it over-states by 5–10 % | ⭐ measured (C13's number) | ⭐ complete — a documented sign error | small, but the variance term built on it is sized from the wrong direction |
