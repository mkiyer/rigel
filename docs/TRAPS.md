...
**an-oracle-column-that-omits-a-population. An oracle arm that fires at full strength on a truth column
missing a whole population is a SUPPRESSION of that population wearing the oracle's name — and it reads
as a triumph.** The allocation arm read `observed_mrna_fragments`, identically 0 on every synthetic
nascent row, so it handed all 6,919 shadow entities a weight of zero and reproduced the retired
`alpha = 0` rule to within 15 %; the fire counter was healthy throughout, because 2,966 annotated rows
were weighted. The sibling of `could-the-arm-have-fired` and its opposite: assert that every population
the truth describes RECEIVES the injection, not merely that the injection ran.
...
`quant_accuracy.py`'s injection arms. *Sibling:* `TRAPS: a-symptom-is-not-a-second-defect`.
